...
A thin REST client that lets a SeisWork install drive a remote SeisWork *server*
(the Flask app in ``seiswork.web.app``): submit pipeline jobs, poll status/logs,
and auto-sync finished-job artifacts to a local mirror. SeisWork still runs fully
standalone — this is only used when a server URL is configured.

ID synchronisation: the server assigns each job/config a UUID; the client keeps a
local registry (``~/.seiswork/registry.json``) mapping those server-assigned IDs
to the server they live on and the local mirror path, so the same ID refers to
the same job on both sides.

Auth: optional bearer token (``Authorization: Bearer <token>``). When the server
runs without a token, none is sent.
"""
...
class SeisWorkClient:
    ...
    def server_info(self) -> dict:
        """Connection info: server_id, urls, auth_required, and the token (the
        server only reveals the token to localhost / an authenticated caller)."""
    ...
    # Scoped to one project: the server only accepts a sync token issued for
    # that cfg_id, so a subscriber given access to one project's data cannot
    # use the same call to read another project hosted on the same server.
    ...
    def pull_bundle(self, job_id: str, cfg_id: str, kind: str = "pipeline",
        ...
        """Download and extract the ZIP bundle for *job_id* (must belong to
        *cfg_id*) to *dest_dir*.

        Default dest_dir: ``sync_dir/<kind>/<job_id>/``
        ...
        r.raise_for_status()
        # Stream to temp file — avoids holding entire ZIP in memory for large jobs.
    ...
    def sync_pull(self, cfg_id: str, job_ids: "list | None" = None, dry_run: bool = False,
        ...
        """Pull new or updated jobs for one project (*cfg_id*) from the server.

        Compares server checksums against locally stored ``.sync_checksum`` marker
        files and only downloads changed / absent jobs.

        Returns a list of result dicts, each with keys:
            id, kind, action (pulled|skip|error), path|reason|error
        """
...
def save_connection(server_url: str, token: str = "", sync_dir: str = "") -> dict:
    """Persist the active server connection so later `seiswork remote ...` calls
    need no --server/--token. Stored in ~/.seiswork/remote.json."""
    SEISWORK_HOME.mkdir(parents=True, exist_ok=True)
