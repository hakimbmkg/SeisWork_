...
def _load_station_df(sta_file: str) -> pd.DataFrame:
    ...
    Supported pipe layouts (FDSN/SeisComP conventions differ in whether a
    location-code column sits between STA and LAT):
    ...
    Layout is auto-detected per file by checking which column offset gives
    valid lat/lon ranges - a fixed offset would misread one layout (LOC
    parsed as LAT -> all-NaN).
...
def _pick_event_key(picks: pd.DataFrame) -> str:
    ...
    event_index, REAL/association -> event_id, some exports -> id). Return
    the first present column so the phase builder joins correctly instead
    of silently linking nothing.
    """
...
def catalog_picks_to_hypodd_phase(cat_df: pd.DataFrame, picks_df: pd.DataFrame,
    ...
    Picks are joined to events by a shared event key. When the located
    catalog has ids that no pick references (e.g. the locator re-ids events
    and drops the association id), the id-join finds nothing and ph2dt would
    later report 0 differential-time pairs. To keep relocation-from-picks
    working, we (1) try every plausible key and (2) fall back to an
    origin-time window join, assigning each pick to the nearest catalog
    event by origin time (within a tolerance), like LOC-FLOW keeps phases
    attached to events through the chain. Warns if nothing links.
    """
    picks = _normalize_picks(picks_df) if not picks_df.empty else pd.DataFrame()
    key   = _pick_event_key(picks) if not picks.empty else ""

    # Lookup from event key value -> catalog event ordinal; detect whether
    # the id-join works at all (any overlap between the two id sets).
    ...
    # Time-window fallback: sort catalog origin times once.
    ...
    if use_time_join:
        ...
        # Assign each pick to the nearest event by origin time (picks fall
        # after the origin; tolerate up to 90 s P/S moveout at local distances).
        TOL_NS = int(90e9)
        picks = picks.copy()
