...
to a system slinktool such as the one SeisComP ships, e.g. on PATH).

Motivation: a station's inventory entry can go stale relative to the live
...
# Persists every CONFIRMED live channel (from a slinktool query that actually
# succeeded) per config, so a station whose true band isn't "BH" — e.g. AAI,
# which only ever transmits SH — still gets the right one when a later query
# times out (a large aggregator SeedLink server won't always answer -Q inside
# the short connect-time budget). Without this, the last-resort fallback
# would force "BH" for AAI too and it would go silent, even though its real
# channel was already discovered once before.
...
def resolve_live_channels(streams: list[tuple], host: str, port: int,
    ...
    (e.g. both BH and HH and SH transmitting at once) only ever gets ONE
    subscribed, not all of them at the same time. Every confirmed result is
    saved to the per-config channel cache (config_station.json) as it's found.

    When slinktool is unavailable or the query doesn't finish in time (a
    large aggregator SeedLink server — thousands of streams from many
    networks — can take 30s+ to enumerate everything via -Q, not worth
    blocking Connect for), each station falls back to its own cached channel
    from a PAST successful query when one exists (e.g. AAI, which only ever
    transmits SH — forcing "BH" for it would go silent even though its real
    channel was already discovered before). Only a station with no live
    query AND no cache entry gets the last-resort "BH" force — deterministic
    single-band is still preferred over a wildcard that could pull in more
    than one simultaneously-live band. Use "Verify Channels"
    ...
    for net, sta, loc, cha in streams:
        ...
        if band:
            ...
            # Not found in THIS query (station offline right now, or a
            # partial/failed enumeration) — fall back to a past confirmed
            # channel for this station before resorting to a blind "BH".
...
def verify_channels(stations: list[dict], host: str, port: int,
    ...
    the cache that a later, short-timeout connect() can fall back on."""
    live = query_streams(host, port, timeout=timeout)
    from seiswork.utils.channels import BAND_PRIORITY
    matched = 0
    mismatched = []
