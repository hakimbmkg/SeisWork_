...
─────────────────────────────────────────────────────────────────────────────
Cloned into SeisWork (by HakimBMKG) from the SeisComP Python distribution
(seiscomp/lib/python/seiscomp/mseedlite.py) — verbatim, license preserved.

Reason: pure-Python record parser (struct.unpack only, no compiled
_core.so/_io.so).  SeisComP's compiled bindings are ABI-incompatible with
the `seiswork` conda env's Python (see seiswork/utils/sds_lite.py for the
lightweight reader built on top of this module, inspired by
BotSeiscompListener/bin/BotListener.py's seiscomp.io.RecordStream pattern:
read records directly, skip what is not needed, decode only what is).
─────────────────────────────────────────────────────────────────────────────
"""
