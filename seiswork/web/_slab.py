...
Generates subduction-slab geometry for whatever Area Of Interest (AOI) is
active in the current SeisWork session, using the official USGS Slab2.0
models (Hayes et al., 2018) hosted on ScienceBase.

Pipeline (region-agnostic, driven by the session AOI bbox):
  ...
  3. generate()          -- clip each grid to the AOI, contour the depth surface
                            ...
                            in km, negative downward), same schema as the
                            historical footages/Contour_slabs.geojson.

The frontend feeds that FeatureCollection into both the 2D cross-section
views and the 3D mesh3d surface, so a single source covers both.

Results are cached per-AOI on disk so re-opening a result is instant.
"""
...
def _load_polygons() -> dict[str, list[tuple[float, float]]]:
    """Parse slab_polygons.txt into {region_code: [(lon, lat), ...]}."""
...
def _collect_links() -> dict[str, str]:
    """Scrape ScienceBase parent item into {region_code: item_url}. Cached."""
    global _link_cache
