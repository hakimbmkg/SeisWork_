...
(() => {
  ...
  function _withCfg(url, sep = '?') {
    return _cfgParam ? `${url}${sep}${_cfgParam}` : url;
  }
  ...
  async function pollPickLog() {
    try {
      const data = await _fetchTimeout(
        `/api/online/picks/recent?since=${_pickSince}&n=100${_cfgParam ? '&' + _cfgParam : ''}`, 5000);
  ...
  async function tick() {
    ...
    const wfPromise = wasSnapshot
      ? _fetchTimeout(_withCfg('/api/online/waveform/snapshot'), 5000)
      : _fetchTimeout('/api/online/waveform/delta', 5000, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ cursors: buildCursors(), cfg_id: _cfgId }),
        });
    try {
      const [st, wfData] = await Promise.all([
        _fetchTimeout(_withCfg('/api/online/status'), 5000),
        wfPromise,
      ]);
  ...
  async function tickSpec() {
    ...
    try {
      const sp = await _fetchTimeout(
        `/api/online/spectrogram/all?window=1800&keys=${encodeURIComponent(keys.join(','))}${_cfgParam ? '&' + _cfgParam : ''}`,
        6000
      );
  ...
  async function tickDenoise() {
    ...
    try {
      const j = await _fetchTimeout(
        `/api/online/waveform/denoised?window=1800&keys=${encodeURIComponent(keys.join(','))}${_cfgParam ? '&' + _cfgParam : ''}`,
        15000);
