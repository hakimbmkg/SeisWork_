...
class OnlineMonitorModule {
  ...
  _startWaveformSSE() {
    this._stopWaveformSSE();
    const es = new EventSource('/api/online/waveform/stream');
  ...
  _renderEventDetail(ev, sdsPath) {
    ...
    this._drawBeachball(document.getElementById('om-evd-beach'), ev.focal);
  ...
  async _fetchEventFocal(ev) {
    ...
    if (note) note.innerHTML = 'Focal (FocoNet): <span style="color:#94a3b8">menghitung polaritas…</span>';
    try {
      ...
      if (d.focal) {
        ...
        note.innerHTML = `Focal (FocoNet): <span style="color:#f59e0b">${d.error || 'unavailable'}</span>`;
        if (wrap) this._drawBeachball(document.getElementById('om-evd-beach'), null);
      }
    } catch (e) {
      if (note) note.innerHTML = `Focal (FocoNet): <span style="color:#f87171">failed: ${e.message}</span>`;
    }
  }
  ...
  _showFocal(ev, focal) {
    ...
    const wrap = document.getElementById('om-evd-beach-wrap');
    if (wrap) {
      // Replace the canvas with a beachball PNG image (matplotlib backend renderer)
      wrap.innerHTML =
        `<img class="om-evd-beach" src="/api/online/realtime/event_focal_png?id=${encodeURIComponent(ev.event_id)}&t=${Date.now()}"
