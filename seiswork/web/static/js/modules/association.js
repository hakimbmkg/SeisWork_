class AssociationModule {
  ...
  static get CFM_FIELDS() {
    return ['min_mag', 'max_mag', 'max_rms', 'min_phase', 'max_gap', 'min_cc',
  ...
  // Recommended Min CC threshold. Derived empirically from cfg 797c0b01 (RunExtJailolo,
  // 7G network): mean pairwise inter-station CC over 850 sampled REAL events was
  // unimodal, mean 0.34, with the lowest 0.5th percentile still at ~0.22-0.24 and no
  // separate low-CC "ghost" cluster visible in this already-associated catalog. 0.15 sits
  // clearly below that entire observed range, so it acts as an outlier floor (won't trim
  // genuine weak events) rather than a general quality filter. Re-tune per network/config —
  // station geometry and frequency content change what "coherent" looks like.
  static get MIN_CC_RECOMMENDED() { return 0.15; }
  ...
  // Min CC is opt-in: disabled by default since it reads waveforms (slow). Checking the
  // toggle enables the field and pre-fills the recommended threshold; unchecking clears it
  // so the generic CFM_FIELDS loop in runCatFilter() naturally treats it as "not filtered".
  toggleMinCC(checked) {
    const el = document.getElementById('cfm-min_cc');
    if (!el) return;
    el.disabled = !checked;
    el.value = checked ? AssociationModule.MIN_CC_RECOMMENDED : '';
  }
