...
class ResultViewerWaveformModule {
  constructor(core) {
    ...
    this.corrStack = null;  // {cc, stack, phase} from /waveform/corr_stack, per-event cache
    // Layout constants
  ...
  reset() {
    ...
    this.eventsAttached = false; this.hoverT = null; this.corrStack = null;
  }
  ...
  drawCanvas(data) {
    ...
    this.specs  = {};
    this.specOn = false;
    this.corrStack = null;  // new event → drop any cached CC/stack from the previous one
    ...
    const ccTog = document.getElementById('rm-wv-cc-tog');
  ...
  async toggleCorrStack(on) {
    const rowEl = document.getElementById('rm-wv-cc-row');
    if (!on) { if (rowEl) rowEl.style.display = 'none'; return; }
    if (rowEl) rowEl.style.display = 'flex';
    if (!this.corrStack && this.data) this.loadCorrStack();
    else { this.renderStackCanvas(); this.renderCCMatrix(); }
  }

  async loadCorrStack() {
    if (!_RM.cfgId || !_RM.activeJob || !this.data) return;
    const infoEl = document.getElementById('rm-wv-cc-info');
    if (infoEl) infoEl.textContent = '⟳ Cross-correlating stations & building stack…';
    try {
      const url = `/api/result/${_RM.cfgId}/waveform/corr_stack?job_id=${_RM.activeJob.job_id}&event_id=${encodeURIComponent(this.data.event_id)}`;
      const res = await fetch(url); const d = await res.json();
      ...
      this.renderStackCanvas();
      this.renderCCMatrix();
    } catch (e) { if (infoEl) infoEl.textContent = '⚠ ' + e.message; }
  }

  renderStackCanvas() {
    drawStackCanvas(document.getElementById('rm-wv-stack-canvas'), this.corrStack && this.corrStack.stack);
  }
  ...
  closeModal() {
    ...
    const ccTogEl = document.getElementById('rm-wv-cc-tog');
...
// ── Shared CC/stack canvas renderers ────────────────────────────────────────
// Standalone (canvas + data in, no `this`) so both the offline Result Viewer
// (above) and the Online Monitor's live event-waveform modal can call the same
// drawing code instead of maintaining two copies — see online-monitor.js's
// toggleCorrStack()/loadCorrStack(), which fetch from the sibling
// /api/online/realtime/event_waveform/corr_stack endpoint and call these directly.

// Small blue↔white↔red diverging scale for CC (-1..+1) — matches the RdBu_r
// convention already used in event_station_stack_xcorr.ipynb's CC matrix plot.
function _ccDivColor(v) {
  const t = Math.max(-1, Math.min(1, v));
  const lerp = (a, b, f) => Math.round(a + (b - a) * f);
  if (t >= 0) {
    const f = t;
    return `rgb(${lerp(248, 178, f)},${lerp(250, 24, f)},${lerp(252, 43, f)})`;
  }
  const f = -t;
  return `rgb(${lerp(248, 33, f)},${lerp(250, 102, f)},${lerp(252, 172, f)})`;
}

function drawStackCanvas(canvas, stack) {
  if (!canvas || !stack) return;
  const W = canvas.parentElement.clientWidth || 400, H = 120;
  canvas.width = W; canvas.height = H;
  const ctx = canvas.getContext('2d');
  ctx.fillStyle = '#0d1626'; ctx.fillRect(0, 0, W, H);

  const grid = stack.rel_grid;
  const t0 = grid[0], t1 = grid[grid.length - 1];
  const toPx = t => ((t - t0) / (t1 - t0)) * W;
  const toPy = v => H / 2 - v * (H * 0.42);

  // t=0 (pick) marker
  ctx.strokeStyle = 'rgba(239,68,68,0.6)'; ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(toPx(0), 0); ctx.lineTo(toPx(0), H); ctx.stroke();

  // Overlay: every station, light gray
  ctx.strokeStyle = 'rgba(148,163,184,0.45)'; ctx.lineWidth = 0.7;
  Object.values(stack.aligned).forEach(arr => {
    ctx.beginPath();
    let fp = true;
    arr.forEach((v, i) => {
      if (v == null) { fp = true; return; }
      const x = toPx(grid[i]), y = toPy(v);
      if (fp) { ctx.moveTo(x, y); fp = false; } else ctx.lineTo(x, y);
    });
    ctx.stroke();
  });

  // Stack: bold accent line on top
  ctx.strokeStyle = '#60a5fa'; ctx.lineWidth = 1.6;
  ctx.beginPath();
  let fp = true;
  stack.stack.forEach((v, i) => {
    if (v == null) { fp = true; return; }
    const x = toPx(grid[i]), y = toPy(v);
    if (fp) { ctx.moveTo(x, y); fp = false; } else ctx.lineTo(x, y);
  });
  ...
  ctx.font = '8px monospace'; ctx.fillStyle = '#64748b'; ctx.textAlign = 'left';
  ctx.fillText(`n=${stack.stations.length}`, 3, 10);
}

function drawCCMatrix(canvas, cc) {
  if (!canvas) return;
  if (!cc) { canvas.width = 0; canvas.height = 0; return; }

  const stations = cc.stations, n = stations.length;
  const LBL = 42, CELL = Math.max(22, Math.min(34, 240 / n));
  const W = LBL + CELL * n, H = LBL + CELL * n;
  canvas.width = W; canvas.height = H;
  const ctx = canvas.getContext('2d');
  ctx.fillStyle = '#0d1626'; ctx.fillRect(0, 0, W, H);
  ...
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < n; j++) {
      const v = cc.cc_matrix[i][j];
      ctx.fillStyle = _ccDivColor(v);
      ctx.fillRect(LBL + j * CELL, LBL + i * CELL, CELL - 1, CELL - 1);
      ctx.font = `${Math.min(9, CELL * 0.32)}px monospace`;
      ctx.fillStyle = Math.abs(v) > 0.55 ? '#0b0e1a' : '#e2e8f0';
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillText(v.toFixed(2), LBL + j * CELL + CELL / 2, LBL + i * CELL + CELL / 2);
    }
  }
  ctx.font = '8px monospace'; ctx.fillStyle = '#94a3b8'; ctx.textBaseline = 'middle';
  for (let i = 0; i < n; i++) {
    ctx.textAlign = 'right';
    ctx.fillText(stations[i], LBL - 4, LBL + i * CELL + CELL / 2);
    ctx.save();
    ctx.translate(LBL + i * CELL + CELL / 2, LBL - 4);
