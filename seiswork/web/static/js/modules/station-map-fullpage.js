...
(() => {
  ...
  async function loadStations() {
    try {
      ...
      const r = await (await fetch(`/api/online/stations?inventory=${encodeURIComponent(invAll.join(','))}`)).json();
  ...
  async function pollPicks() {
    try {
      const data = await (await fetch(`/api/online/picks/recent?since=${pickSince}&n=100`)).json();
  ...
  async function renderStationModal() {
    ...
    const results = await Promise.all(channels.map(cha =>
      fetch(`/api/online/waveform?station=${net}.${sta}..${cha}`)
  ...
  async function pollEvents() {
    try {
      const r = await (await fetch('/api/online/realtime/events')).json();
