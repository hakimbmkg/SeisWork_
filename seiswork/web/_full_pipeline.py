...
Chains the whole post-pick pipeline on a picks.csv by driving the local GUI
server's /api/pipeline/run endpoint (reusing the tested _build_pipe_cfg +
_pipe_worker), feeding each step's output catalog into the next:

    picks.csv ─► assoc/gamma ─► locate/nlloc ─► magnitude/ml ─► relocation/hypodd

Each step's PIPE_DIR/<job>/status.json carries state + result_file; we poll state
and chain on result_file (no filename guessing). Auth uses the server bearer
token at ~/.seiswork/token.
...
PIPE_DIR = Path("/home/hakim/apps/seiswork/seiswork/web/tmp/pipeline")
TOKEN    = ""
try:
...
# step → (method, params). assoc uses GaMMA DBSCAN pre-clustering so it scales to
# a FULL YEAR of picks (~millions) in one pass: DBSCAN groups picks by space-time
# first, then BGMM associates within each cluster (vs O(N) BGMM over everything).
# Other steps take GUI defaults (jailolo NLLoc grid, ML a=1.110, hypoDD catalog).
_ASSOC_DBSCAN = {
    "use_dbscan"        : True,
