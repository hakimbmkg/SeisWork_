...
Architecture:
  RealtimePicker: 1 thread, 1 PhaseNet model (seisbench) loaded once; each
                       cycle loops all active stations and calls
                       model.annotate() per station within the same process
                       (not N separate OS processes - process-level
                       parallelism for GPU-bound PhaseNet was shown to add
                       no throughput, only overhead from N model loads and
                       N CUDA contexts).
  RealtimeAssociator: separate thread, feeds the rolling pick buffer into
                       GaMMA (self-contained, gives its own lat/lon/depth via
                       BGMM+BFGS, no NLLoc grid needed). On a new event it
                       computes ML magnitude from the ring buffer directly
                       (not MLMagnitude.run(), which needs an on-disk SDS
                       that live-only sessions don't have).

Focal mechanism (FocoNet/SKHASH) is not implemented here: it needs sds_path
(disk) like MLMagnitude does. Events are stored without one (best-effort).
"""
...
PICK_OVERLAP_S   = 6.0    # small overlap with the previous cycle (catches picks near the edge)
PICK_INTERVAL_S  = 5.0
RESCAN_WINDOW_S  = 180.0  # one-shot rescan on (re)start, recovers events missed during downtime (~3 min)
...
# ── Realtime associator backend ─────────────────────────────────────────────
# REAL (travel-time grid, deterministic) is far more stable than GaMMA, whose
# GMM is refit every cycle so cluster assignments can flip between cycles.
# REAL associates and gives a rough location; NonLinLoc then refines it
# (iasp91 grid). Default is "real"; start() can override to "gamma" (legacy path).
ASSOC_BACKEND_DEFAULT = "real"

# ── NLLoc refinement (refines the GaMMA/REAL hypocenter per new event) ──────
# GaMMA only does association + a rough location (BGMM+BFGS); the final
# location comes from NonLinLoc using the global IASP91 travel-time grid.
# The grid is built once (Vel2Grid/Grid2Time) and just referenced here - this
# module doesn't build it. Refinement runs per new event, not every cycle, to
# keep the loop light. If NLLoc fails or the binary is missing, fall back to
# the GaMMA/REAL location (event still kept).
NLLOC_REFINE_ENABLED = True
NLLOC_PROFILE  = "iasp91"   # global grid model_prefix (see *.time.hdr)


def _default_nlloc_grid_dir() -> str:
    """Global IASP91 grid bundled in the repo (config/nlloc_grids/global) for
    portability, instead of the old hardcoded ~/apps/NLLoc path. Falls back to
    the old location if the bundle is missing (older checkout)."""
    ...
    if (bundled).is_dir() and any(bundled.glob(f"{NLLOC_PROFILE}.P.*.time.hdr")):
        return str(bundled)
    # Bundle missing (old checkout): disable NLLoc refinement gracefully
    # instead of pointing at a personal path that won't exist elsewhere.
    return ""


NLLOC_GRID_DIR = _default_nlloc_grid_dir()

SDS_RETAIN_DAYS = 1   # default retention (days) for work/online_sds, overridable
                      # per config via "archive_days" (Edit Configuration in the
                      # GUI). This is the on-disk SDS archive for historical event
                      # waveform review, separate from the in-memory live ring
                      # buffer in _seedlink_live.py (_BUFFER_SECONDS, fixed at 1h,
...
def find_slarchive() -> str:
    """Prefer the repo-bundled slarchive (compiled by install.sh step 3, a
    known-good version independent of any SeisComP install), else fall back
    to whatever's on PATH (e.g. SeisComP's own copy)."""
...
def _seiscomp_sds_is_fresh(sc_sds: str, max_age_hours: float = 2.0) -> bool:
    """True if the SeisComP SDS has a file modified within max_age_hours,
    scoped to this year's folder only. If True, scarchive is active and
    slarchive isn't needed (avoids duplicate data).

    Scoped to the current year so an unrelated job writing into an old year
    folder of the same archive (e.g. a historical backfill sharing the SDS
    path) can't make a stale archive look fresh."""
    if not sc_sds or not Path(sc_sds).exists():
        return False
    year_dir = Path(sc_sds) / str(datetime.now().year)
    if not year_dir.exists():
        return False
    cutoff = time.time() - max_age_hours * 3600
    for f in year_dir.rglob("*.D"):
        try:
...
def _slarchive_selector(streams) -> str:
    """Build the slarchive -S argument ('NET_STA,NET_STA,...') from a stream
    list. Items can be 'NET.STA.LOC.CHA', 'NET.STA', or a (net, sta) tuple.

    Without -S, the SeedLink server sends no packets: slarchive just connects,
    times out after 600s repeatedly, and the SDS stays empty - why event
    waveforms sometimes never get archived."""
    if not streams:
        return ""
    seen = []
    for s in streams:
...
def _kill_stray_slarchive(sds_dir: str) -> int:
    """Kill any orphaned slarchive process still writing to the same SDS. If
    the Python server restarts, the global _SLARCHIVE_PROC is lost but the
    child slarchive stays alive, so they pile up and fight over the SeedLink
    connection. Scans /proc (Linux); best-effort, skipped on other OSes."""
    killed = 0
    try:
        my_pid = os.getpid()
        proc_root = Path("/proc")
...
def start_slarchive(base_dir: str, host: str, port: int,
    ...
    """Start slarchive into the local SDS, but only when the SeisComP SDS is
    not fresh (if scarchive is already active, skip to avoid redundancy).
    `streams`: active station list, becomes the -S selector (required for
    data to flow). `retain_days`: archive retention from config's
    "archive_days" field, defaults to SDS_RETAIN_DAYS.
    Returns True if started, False if already running, 'skipped' if the
    SeisComP SDS is fresh."""
    ...
    with _SLARCHIVE_LOCK:
        ...
        # Clean up any orphaned slarchive from earlier restarts writing to this SDS
        ...
        # Fallback: if slarchive isn't bundled or on PATH, write the SDS via ObsPy
        ...
        try:
            with open(log_file, "a") as lf:
                # start_new_session=True: slarchive gets its own session instead of
                # being a direct Flask child, so a Flask restart (HUP) won't turn
                # it into a zombie.
...
def _sds_cleanup_loop(sds_dir: str, max_days: int = SDS_RETAIN_DAYS):
    ...
    waveforms stay openable without storage filling up.
...
class RealtimeAssociator:
    ...
    def __init__(self, picker: RealtimePicker):
        ...
        # Global IASP91 grid is generated at runtime for the active network,
        # so NLLoc refinement works for any region, not just the bundled one.
        ...
        for _, ev in catalog_df.iterrows():
            ...
            if picks_gamma is not None:
                try:
                    ...
                    # Store P-pick timing for FocoNet, no live pick log needed
    ...
    def _compute_magnitude(self, event: dict) -> float | None:
        ...
        now = time.time()
        # The window must cover the event time: events are often detected
        # minutes after origin, so request a window as wide as the event age
        # + coda (capped at 30 min; the ring buffer keeps 1 day).
        # Widened per-station below (win_end) so far stations' S-wave/coda -
