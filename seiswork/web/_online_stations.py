...
Parse a StationXML / FDSNXML inventory → a station list for Online mode.
"""
...
def get_stations(
    ...
    # A station can have SEVERAL epochs in the XML (moved location, re-surveyed
    # coordinates, metadata revision, ...) — each is a separate Station element
    # with its own start/end date. Naively taking "whichever comes first in the
    # file" can pick a stale, superseded epoch. Group by (net,sta) and keep only
    # the most current one: prefer an epoch with no end_date (still active),
    # otherwise the one with the latest start_date.
    ...
    for (net_code, sta_code), pairs in epochs.items():
        ...
        # Location code of the preferred channel — REQUIRED for response
        # removal / magnitude. Stations mix '' and '00'; if we feed the wrong
        # loc to remove_response it raises "No matching response" and ML falls
        # back to raw counts (→ M12 over-estimate). Pick the most recent epoch.
        pref_chs = sorted(
