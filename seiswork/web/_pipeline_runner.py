...
if __name__ == "__main__":
    ...
    # Copy picks from the input catalog dir → this job_dir so that downstream
    # steps (NLLoc obs, HypoDD phase.dat) always find picks alongside the
    # output catalog of this step.
    if input_file:
        import shutil
    ...
    try:
        ...
        # ── Location ───────────────────────────────────────────────────────────
            ...
            if method == "nlloc":
                ...
                # Station list → lets NLLocLocator auto-build a travel-time
                # grid (sized to the network's spread) when none exists yet
                # at the configured grid_dir, instead of requiring a
                # pre-built one (see NLLocLocator._ensure_grid).
                stations = None
                try:
        ...
        # ── Mechanism (SKHASH first-motion + FocoNet deep-learning) ─────────────
        elif step == "mechanism":
            ...
            # "event_index" instead — normalise both P and S frames the same way.
