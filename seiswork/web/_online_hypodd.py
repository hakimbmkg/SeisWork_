...
def _write_event_and_phase(events: list[dict], sta_written: set[str],
    ...
    """Write HypoDD event.dat and phase.dat.
    ...
    with open(event_path, "w") as ef, open(phase_path, "w") as pf:
        for ev in events:
            ...
            # event.dat: same format as catalog_picks_to_hypodd_phase
            ef.write(
...
def _parse_hypodd_reloc(reloc_file: str,
    ...
    """Parse hypoDD.reloc into a list of events with updated coordinates.
    ...
    with open(reloc_file) as f:
        for line in f:
            ...
            try:
                ...
                # Ex, Ey, Ez (horizontal/vertical error) in columns 7-9
...
"""
        ...
        # Save to online_catalog/<cfg_id>/hypodd_reloc.json
