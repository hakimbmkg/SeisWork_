...
def parse_dtcc(path: str) -> dict:
    """Parse dt.cc → dict with arrays ev1, ev2, dist_km, sta, dt_s, cc, phase."""
...
def parse_event_dat(path: str) -> dict:
    """Parse event.dat (ph2dt output) → {id: {lat, lon, dep, mag, time_str}}.
...
def parse_hypodd_pha(path: str) -> dict:
    """Parse hypoDD.pha → {eid: {ts_origin(str), picks: {sta: {P: tt, S: tt}}}}.
...
def _parse_dtct(path: str) -> dict:
    """Parse dt.ct (catalog differential times) → same schema as dt.cc dict.
