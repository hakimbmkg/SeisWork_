...
# ── Paths ──────────────────────────────────────────────────────────────────────
# app.py lives at seiswork/web/app.py → parent×3 = project root
...
app.config["MAX_CONTENT_LENGTH"] = 256 * 1024 * 1024  # 256 MB upload limit (full StationXML w/ response stages can be tens of MB)
...
# SeisWork can run fully standalone (default) or as a *server* that remote
# *clients* (CLI or Web GUI mirror) connect to. Each server has a stable
# server_id so job/config UUIDs sync coherently across client and server. Auth is
# an optional bearer token (SEISWORK_TOKEN); empty token = auth disabled (LAN).
import secrets
import socket
...
# Token: env wins; then a runtime token set from the GUI (~/.seiswork/token);
# then config federation.token; empty → auth disabled. The runtime file lets the
# operator set/generate a token from the GUI and have it survive restarts without
# rewriting (and reformatting) config.yaml.
...
# A second, identical GUI can be launched on a separate port (default 3346) that
# *mirrors* the main monitor read-only — same template, but every /api/online/*
# read is proxied to the upstream main server (default 127.0.0.1:5000) instead of
# touching this process' own (empty) _LIVE_SESSION. Mutating calls that would
# disturb the real monitor (connect/disconnect, realtime start/stop, inject,
# config edits, …) are intercepted locally and never forwarded, so the mirror can
# never alter the upstream. The viewer is auto-spawned by the main server's run()
# and self-restarts on code edits via the usual _watch_py_and_restart loop.
...
def _viewer_intercept():
    """In viewer mode, satisfy /api/online/* from the upstream main server and
    block anything that would mutate it. Returns a Flask response to short-circuit
    the request, or None to let normal (local) handling proceed."""
    if not VIEWER_MODE:
        return None
    p = request.path
    ...
    # connect / realtime-start → don't forward; report the upstream's live state
    # so the existing frontend flow lights up the dashboard against the mirror.
    if p in _VIEWER_FAKE_OK and request.method == "POST":
        ...
        return jsonify(data)
    # disconnect / realtime-stop / inject / config edits / uploads / … → local
...
def _viewer_fwd_headers() -> dict:
    """Forward the caller's bearer token (and fall back to this server's own) so
    upstream auth — when enabled — still passes through the mirror."""
    h = {}
...
def _viewer_proxy(path: str):
    """Proxy one /api/online/* read to the upstream main server, preserving the
    ...
    # SSE endpoint → stream-proxy: forward as chunked transfer so the viewer
    # receives push events without the 30s timeout killing the connection.
...
# sync (/api/sync/*) is the channel a *subscriber* uses to pull one project's
# data, so it must not accept that same server-wide token — otherwise any
# subscriber with access to one project could read every project hosted on
# this server. Each cfg_id gets its own token, generated on first use and
# persisted next to its meta.json.
...
def _cors_headers(resp):
    ...
    # The HTML shell must always be fresh (it carries the ?v= cache-bust tokens),
    # but the assets themselves are cacheable: app files change their ?v= token
    # when edited (so the browser refetches on the next live-reload), and vendor
    # files are immutable. Caching them makes a normal reload fast instead of
    # re-downloading everything (esp. the big Plotly bundle) every time.
    if request.path.startswith("/static/"):
        # Versioned assets (…?v=<mtime>) are safe to cache long — the token flips
        # when the file changes. Un-versioned assets (favicons, native-bridge.js,
        # images referenced without ?v=) must always revalidate, otherwise an edit
        # is masked for up to 7 days by the browser / WKWebView disk cache.
        if request.args.get("v"):
            resp.headers["Cache-Control"] = "public, max-age=604800"   # 7 days
        else:
            resp.headers["Cache-Control"] = "no-cache, must-revalidate"
    elif "text/html" in resp.headers.get("Content-Type", ""):
        # EVERY rendered page — the main shell AND every full-page view
        # (/online/waveform-fullpage, station-map, catalog-map, pipeline-flow) —
        # must be fresh. They carry ?v= tokens and inline theme CSS; a cached copy
        # (esp. in WKWebView's persistent store) masks edits and leaves an
        # undocked pop-up window showing the OLD light-only stylesheet.
...
#   2. when offline — or the upstream 404s / times out — we return a neutral
#      transparent placeholder with HTTP 200 instead of a broken tile, so there
#      is never a "404 / not found" without internet.
# The provider table lives server-side, which also hides the axis-order quirks
...
# 1×1 fully-transparent PNG — the browser scales it to the 256px tile slot, so an
# unavailable tile simply shows the map container background (no broken image).
...
def help_docs():
    ...
    build (the doc's own in-page toggle links here). The file is gitignored
    (a local build artifact), so a fresh checkout may not have it yet; that
    path shows how to build it rather than failing with a raw 404."""
...
def api_health():
    ...
    server identity and whether a token is required (before sending one)."""
    return jsonify({
        "ok"           : True,
        "service"      : "seiswork",
        "server_id"    : SERVER_ID,
...
def api_server_restart():
    """Restart THIS server in place (os.execv → same port). Lets an operator recover
    a wedged server from the GUI without SSH. Allowed only from localhost or with the
    current token (never by an unauthenticated remote). Refuses while a job is running
    unless force=true, so a restart never silently kills a running pipeline. The reply
    is sent first; the re-exec fires ~0.6s later (the frontend polls /api/health, then
    reloads). The call the frontend makes targets location.origin so it always restarts
...
def api_online_stations():
    """Parse a StationXML inventory → station list [{net,sta,lat,lon,name,channels}]."""
...
def api_online_connect():
    ...
    # its own client thread; every source may bring its own sensor XML so the
    # merged station set (and thus incoming data) grows beyond a single server.
    ...
    for src in (data.get("sources") or data.get("extra_sources") or []):
        ...
        # Resolve each station to its single best LIVE band — a station with
        # several bands simultaneously live (e.g. BH + HH + SH all at once)
        # would otherwise get every one of them subscribed at connect time
        # (see the orientation-wildcard fallback in _seedlink_live.py).
        try:
    ...
    # Same single-best-live-band resolution for the primary source — skipped
    # when this is clearly a resume (already connected to this exact host:port)
    # so a slinktool query (several seconds) doesn't run on every idle
    # reconnect/resume, only on a connection that's actually changing.
    ...
    if not _is_resume:
        try:
            ...
            pass
    # On-disk SDS roots to refill the waveform ring buffer from history on a
    # (re)start — so the panel resumes from slarchive instead of starting empty
    # when the SeedLink server serves no time-window. Prefer the SeisComP SDS
    ...
    # Mark the active cfg_id → per-config catalog
    if cfg_id:
        from seiswork.web._realtime_pipeline import set_active_cfg_id
        set_active_cfg_id(str(BASE_DIR), cfg_id)

    # Register the scautopick→trigger pick bridge when an agent is provided (online config
    # already synced to SeisComP). Best-effort — a failure does not fail the connect.
...
def api_online_status():
    return jsonify(_LIVE_SESSION.status())
...
def api_online_waveform():
    ...
    return jsonify({"station": station, "points": _LIVE_SESSION.get_buffer(net, sta, loc, cha)})


@app.route("/api/online/waveform/all", methods=["GET"])
def api_online_waveform_all():
    """Every active stream at once — the live waveform panel, stacked per station."""
    return jsonify({"streams": _LIVE_SESSION.get_all_buffers()})
...
def api_online_waveform_snapshot():
    ...
    connect. Afterwards the frontend uses POST /delta with a per-stream cursor to
    fetch only new data. session_epoch lets the client detect a server restart."""
    import time as _time
    return jsonify({
        "streams"      : _LIVE_SESSION.get_snapshot(bin_s=1.0, win_s=1900.0),
        "server_time"  : _time.time(),
        "session_epoch": _LIVE_SESSION.session_epoch,
    })
...
def api_online_waveform_delta():
    ...
    clock) — a small payload. Body: {"cursors": {"NET.STA.LOC.CHA": last_t, ...}}.
    A stream missing from cursors is treated as new → sent the full window. The
    client compares session_epoch to detect a server restart (re-snapshot)."""
    body    = request.get_json(silent=True) or {}
    cursors = body.get("cursors") or {}
    streams, srv_time = _LIVE_SESSION.get_delta(cursors)
    return jsonify({
        "streams"      : streams,
...
def api_online_waveform_denoised():
    ...
    frontend throttles the polling). The model loads lazily in the background;
    until it is ready the response carries ready=False so the frontend keeps the
    raw trace on screen and shows a 'loading denoiser…' hint."""
    import numpy as np
    ...
    for key in keys[:80]:               # hard cap: one request can't blow up
        ...
        try:
            t, v = _LIVE_SESSION.get_window(net, sta, loc, cha, seconds=window_s)
            sr   = _LIVE_SESSION.get_sampling_rate(net, sta, loc, cha)
        except Exception:
            continue
...
def api_online_waveform_stream():
    """SSE waveform push stream — the server sends a delta every ~2s without the
    client needing to poll. The client opens an EventSource once; the server pushes
    new data as it arrives. There is no client-side timeout since the connection
    stays alive (long-lived HTTP). This makes the waveform view independent of the
    user's internet speed: a slow connection still receives data, just later — not disconnected.
    ...
    def _generate():
        ...
        # ── Snapshot awal ────────────────────────────────────────────────────────
        try:
            snap = _LIVE_SESSION.get_snapshot(bin_s=1.0, win_s=1900.0)
            for s in snap:
                if "last_t" in s:
                    cursors[s["key"]] = s["last_t"]
            sent_epoch = _LIVE_SESSION.session_epoch
            payload = _json.dumps({
                "type": "snapshot",
                "streams": snap,
                "server_time": _time.time(),
        ...
        while True:
            ...
            try:
                ...
                cur_epoch = _LIVE_SESSION.session_epoch
                if cur_epoch != sent_epoch:
                    cursors.clear()
                    sent_epoch = cur_epoch
                    snap = _LIVE_SESSION.get_snapshot(bin_s=1.0, win_s=900.0)
                    for s in snap:
                        if "last_t" in s:
                            cursors[s["key"]] = s["last_t"]
                    payload = _json.dumps({
                ...
                payload = _json.dumps({
                    ...
                    "sl_connected": _LIVE_SESSION.connected,
                })
                yield f"data: {payload}\n\n"
...
def api_online_spectrogram_all():
    ...
    specs = _LIVE_SESSION.get_all_spectrograms(window_s=window_s, only_keys=only_keys)
...
def api_realtime_start():
    ...
    associator (GaMMA, self-contained, no NLLoc grid). Opt-in — never auto-enabled
    on a plain connect, since it's CPU/GPU-intensive."""
    if not _LIVE_SESSION.connected:
        return jsonify({"error": "No active live session — connect first"}), 400

    _inv_req = (request.get_json(silent=True) or {}).get("inventory")
    # Merge every inventory of the session (multi-seedlink) — the picker and
    # associator then see stations from ALL sources, not just the primary one.
    ...
    # refreshed afterwards (RealtimePicker.start()/RealtimeAssociator.start()
    # are no-ops while already running) — filtering to "active right now" used
    # to permanently freeze the picker to however many of 470+ SeedLink
    # streams happened to have sent their first packet within the first few
    # seconds after Connect (often as few as ~10), silently ignoring every
    # station that came online afterwards for the rest of the session.
    # _run_cycle()/get_window() already skip a station gracefully when it has
    # no data yet, so including all of them up front is safe — each one joins
    # in naturally on whatever cycle its first packet actually arrives.
    #
    # The channel picked per station must NOT come from the inventory's
    # `default_channel` alone — that's the XML's declared band, which for a
    # lot of stations (e.g. IA.* declared BHZ) does not match what the
    # station is ACTUALLY transmitting live (e.g. SHZ), a mismatch
    # resolve_live_channels() already corrects for at connect() time and
    # persists in config_station.json. Reusing that same cache here means a
    # station whose live band differs from its XML default still gets
    # picked — confirmed live: of 397 currently active streams, only ~111
    # had inventory default_channel == the real live channel; the rest
    # (mostly IA BHZ-vs-SHZ) were silently invisible to the picker before
    # this fix, capping it at ~111/470 regardless of how many stations were
    # truly streaming.
    ...
    # SeisComP SDS archive (Task 2) — from the request, else auto-detect the local install.
    ...
    # Monitoring-area region → associator center (REAL lat_center + NLLoc TRANS
    # origin = box midpoint). RealtimeAssociator._region_cfg honors this override.
    if _area:
...
def api_inject_seiscomp():
    """Inject picks SeisWork (PhaseNet) ke SeisComP via scdispatch SCML.
    Agency ID = 'SeisWork', methodID = 'PhaseNet-AI' — tampil di SeisComP picker list.
    Body JSON: { agency_id, method_id, since }
    """
    import tempfile
    ...
    # Build SCML picks — SeisWork sebagai agency terpisah di SeisComP
...
def _iso_z(dt_val) -> str | None:
    """Normalize a datetime/epoch/ISO string → UTC 'YYYY-mm-ddTHH:MM:SS.ffffffZ'.
    Timezone-correct: a tz-aware value (e.g. '...+07:00') is CONVERTED to UTC, not
    just stripped — so every time SeisWork emits (SCML, catalog, waveform windows)
    is genuinely UTC even when the host runs in a local zone like WIB."""
...
def _build_events_scml(events: list[dict], wf_map: dict,
    ...
    for ev in events:
        ...
        for pid, phase in arrivals:
            ...
            # (a nested <code> is silently dropped → empty phase_code in the DB).
...
def online_pipeline_flow():
    """Halaman Pipeline Flow Monitor — visualisasi alur realtime SeisWork."""
...
def api_realtime_picker_stations():
    ...
    # Sort: picks terbaru → ada data → alphabetical
    ...
    if _sta_req:
        ...
        if len(_t_arr) > 0:
            ...
            # Probability simulation: Gaussian peaks from the picks
            # Evaluate on a 200-point grid to render the curve
            _N_PROB = 200
            _t_prob = [_t_ds[0] + i * (_t_ds[-1] - _t_ds[0]) / (_N_PROB - 1)
                       for i in range(_N_PROB)]
            _SIGMA  = 0.4   # lebar Gaussian ~ PhaseNet resolution
...
def api_realtime_event_waveform():
    """Waveform of the nearest stations around the event time + their picks — for the
    'view pick results' modal. ?id=<event_id>. Window [t0-15s, t0+75s], the 12
    ...
    if ev is None:
        # fall back to the on-disk catalog (events from a previous session / server restart)
        from seiswork.web._realtime_pipeline import load_catalog
    ...
    try:
        ...
        # honours an explicit offset (…+07:00) — both normalise to the same UTC
    ...
    # Fixed window — the ring buffer is a fixed ~1 h span, so requesting a window
    # scaled by (now − t0) gained nothing and, for an event hours old (or one whose
    # OT was mis-parsed into the future), produced an absurd span. The event slice
    # is always [t0-pre, t0+post]; older data comes from the SDS below.
    win = pre + post + 10

    # ── Only stations associated with this event ─────────────────────────────
    # Authoritative sources: ev["stations"] (NET.STA from GaMMA/association) +
    # ev["p_picks"] (stored P picks). Displayed picks = ONLY picks
    # from associated stations, not every pick inside the time window.
    # If ev["stations"] is empty (old event without the field), fall back to 12 stations.
    ...
    # Displayed picks = ONLY from ev["p_picks"] (stored during GaMMA association).
    # The pick log is NOT used — it holds every pick in the window, not just associated ones.

    streams = []
    # Prioritas SDS:
    # 1. Main SeisComP SDS (full archive, SeisWork acting as a SeisComP listener)
    # 2. online_sds (slarchive, for data not yet in the SeisComP SDS)
    ...
    if _sc_fresh:
        sds_path = _sc_sds          # active SeisComP SDS (scarchive running)
    elif Path(_online_sds).exists() and any(Path(_online_sds).iterdir()):
        sds_path = _online_sds      # online_sds from slarchive (default when scarchive is inactive)
    elif _sc_sds and Path(_sc_sds).exists():
        sds_path = _sc_sds          # fall back to SC SDS even if not fresh (old archive)
    else:
    ...
    for s in rows:
        ...
        if len(vv) >= 32:
            try:
                ...
                if _nperseg >= 8:
                    ...
                    # downsample for a compact payload: max 80 time bins × 60 freq bins
        ...
        # Downsample the trace for display (max 1200 points — more detail than the old 600)
    ...
    msg = None
    if not streams:
        # Accurate diagnosis: waveforms may be missing because (a) the event predates
        # the live-session start → absent from the ring buffer, AND (b) missing/gappy in SDS.
        # The old message always blamed SDS even when the session was the real cause.
...
def _latest_sds_day(p: Path, cap: int = 8000) -> str | None:
    """Last day (YYYY.DDD) of an SDS archive — search only the LATEST year folder
    (cheap, does not sweep the whole archive)."""
    max_year = datetime.now().year + 1  # guard against bogus future dirs from a station with a bad clock/GPS lock
    try:
...
def _sds_summary(sds_dir: str, fresh_hours: float = 2.0,
    ...
    full=True  → compute n_files+size (for small archives like online_sds).
    full=False → large archive (SeisComP, hundreds of GB): NEVER sweep every file;
                 'fresh' uses _seiscomp_sds_is_fresh (checks *.D folders, early-exit
                 → accurate & cheap), n_files/size are skipped."""
    import time as _t
...
def api_online_sds_status():
    ...
    external scarchive is writing it (SeisWork only reads, slarchive is SKIPPED);
    otherwise → SeisWork archives it itself via slarchive into work/online_sds;
    if detection isn't running yet → ring buffer only (RAM)."""
    from seiswork.web._realtime_pipeline import (
...
def api_realtime_event_focal():
    ...
    waveform is archived. ?id=<event_id>. The result is cached in event['focal']. Beachball PNG
    ditulis ke work/online_focal/<id>.png (dilayani route event_focal_png)."""
    from obspy import UTCDateTime
    ...
    # Inventory + SDS (sumber polaritas)
    inv_path = getattr(assoc, "_inventory_path", None) or \
               (_LIVE_SESSION.inventory_path if _LIVE_SESSION.connected else None) or \
               find_default_inventory()
    ...
    if stored_p:
        ...
        for pp in stored_p:
            try:
                ts = pp["pick_time"]
                # GaMMA timestamps are stored with a space instead of T plus a timezone
                # "2026-06-26 01:45:11.375000+00:00" → UTCDateTime fails to parse it
                # use datetime.fromisoformat() as a bridge
                try:
                    pt = UTCDateTime(ts)
                except Exception:
    ...
    # short-period sensors (e.g. IA has several SHZ-only sites alongside HHZ/
    # BHZ ones — see seiswork/utils/channels.py) — borrowing one station's
    # default_channel for the whole event meant any station whose actual
    # band differs from that borrowed one silently read 0 waveform samples,
    # so polarities read for 0 stations even when the data was archived fine.
    # compute_polarities()/compute_event_focal() already auto-detect the
    # right band PER STATION (HH>BH>EH>SH priority) when channel=None.
    ...
    ev["focal"] = focal   # cache di event in-memory (referensi assoc.events)
    from seiswork.web._realtime_pipeline import update_catalog_focal
    try:
        update_catalog_focal(str(BASE_DIR), eid, focal)   # persist lintas restart
    except Exception:
        pass
...
def api_realtime_event_focal_png():
    """Layani beachball PNG FocoNet utk 1 event (dihasilkan event_focal)."""
...
def api_realtime_catalog():
    """Persistent event catalog (from disk) — survives restarts. ?limit=N."""
    from seiswork.web._realtime_pipeline import load_catalog
    try:
...
def api_realtime_catalog_csv():
    """Unduh katalog event sebagai CSV."""
    import csv, io
...
def api_online_reloc_status():
    ...
    if status.get("state") == "done":
        ...
        # In-memory _reloc_status is empty (e.g. right after a server restart) — restore
        # the last run's result from disk so the relocation bar does not vanish
        # even though a valid result is already stored.
        result = load_reloc_catalog(str(BASE_DIR), cfg_id=cid)
        if result:
...
def api_online_picks_recent():
    ...
    try:
        ...
        n = 200
    picks = _LIVE_SESSION.get_pick_log(since=since, n=max(1, min(n, 500)))
    return jsonify({"picks": picks, "total": _LIVE_SESSION._n_picks_total})


@app.route("/api/online/psd", methods=["GET"])
def api_online_psd():
    """Power Spectral Density (Welch) for a station from the live buffer — via obspy.
    When an inventory is available, the instrument response is removed → PSD in (m/s)²/Hz
    (acceleration-like dB). Otherwise, raw-counts PSD. ?net= &sta= [&cha=]"""
    ...
    try:
        ...
        # Remove response → m/s, then acceleration-like PSD (multiply by ω²) when an inventory exists
        removed = False
        ...
        data = st[0].data
        nperseg = int(min(len(data), max(256, sr * 20)))  # ~20 s segmen
        f, Pxx = _welch(data, fs=float(sr), nperseg=nperseg,
                        noverlap=nperseg // 2, scaling="density")
        # Buang DC; konversi ke dB (10·log10). +eps hindari log(0).
        mask = f > 0
...
def _fast_station_count(path: str) -> int:
    ...
    upload preview count, so an epoch-level count is enough. Robust against a tag
    split across read-buffer boundaries: the carry is (len(needle)-1) bytes, so no
    complete match ever lies wholly inside it → no missed or double-counted tag."""
    total = 0
...
def upload_online_inventory():
    ...
    # Previously this read the whole file into a str AND then called get_stations()
    # → ObsPy read_inventory, a full parse of every response stage (~1s per 20 MB)
    # just to count stations. That parse was the reason large StationXML uploads
    # felt slow; _fast_station_count is ~90x faster and enough for a preview.
...
def api_agent_status():
    ...
    # Ping /status without auth (agent replies 401 when a token is required, but the port is open)
    try:
        _ur.urlopen(f"{agent_url}/status", timeout=2)
        running = True
    except _ue.HTTPError as e:
        running = (e.code in (200, 401))  # 401 = port terbuka, auth diperlukan
    except Exception:
        running = False
...
def create_online_config():
    ...
    meta = {
        ...
        # (fixed, in-memory, ~1h): this controls how long historical event
...
def update_online_config(cfg_id):
    """Update seedlink host / agent URL / sync status setelah wizard step 2."""
    cfg_dir = CONFIGS_DIR / cfg_id
...
def _norm_region(region: dict) -> dict | None:
    """Validate/normalize a drawn bbox → full region dict with center + span.
...
def _config_region(cfg_id) -> dict | None:
    ...
    (created via Set Area). None otherwise → callers keep the old behavior."""
    if not cfg_id:
        return None
    try:
...
def _streams_in_region(streams, inventory: str, region: dict):
    ...
    if not inventory or not Path(inventory).exists():
        return streams   # no coords to check against → don't silently drop all
    try:
...
def _build_area_grid_bg(cfg_dir: Path, stations: list, center):
    ...
    nlloc_grid_status ∈ {building, ready, failed} + nlloc_grid_dir."""
    meta_file = cfg_dir / "meta.json"
...
# Edits to templates/index.html or static/{js,css} appear in the browser within a
# second — no Ctrl+Shift+R, no manual restart. The page opens an SSE stream; when
# any front-end file's mtime changes the server pushes a new token and the page
# reloads itself. Python edits trigger a graceful in-process re-exec, but only
# while no job is running (so long pipelines are never killed mid-flight).
...
def _reexec_server():
    ...
    os.execv inherits open FDs — including werkzeug's listening socket, which would
    keep holding the port and make the fresh bind fail with 'Address already in use'.
    Mark every non-std FD close-on-exec so the socket is released as the new image
    loads and can rebind cleanly. Shared by the idle live-reload loop and the manual
    ...
    for fd in range(3, 4096):
        try:
            ...
            pass
    # Prefer sys.orig_argv (Py3.10+): it preserves the EXACT original launch, incl.
    # '-m seiswork.web.app' for the viewer child. Re-running that via a bare
    # 'python <app.py path>' would drop the package root from sys.path →
    # "ModuleNotFoundError: No module named 'seiswork'". Fall back for older Pythons.
...
def _slugify_cfg(name: str) -> str:
    """Config name → URL slug (e.g. 'raspyshake' → 'raspyshake', 'PALU-SIGI' →
    'palu-sigi'). Used for the per-config mirror URL (3346/<slug>)."""
    return re.sub(r"[^a-z0-9]+", "-", str(name or "").lower()).strip("-")


def _resolve_cfg_ref(ref: str):
    """ref di URL (cfg_id ATAU slug nama) → (cfg_id, name, meta) | None."""
    ref = (ref or "").strip()
    if not ref:
        return None
    # 1) cocokkan cfg_id persis (folder ada)
    d = CONFIGS_DIR / ref
...
def _inventory_meta(inv_path: str) -> dict:
    """Networks + stations (NET.STA) from an inventory (FDSN StationXML) — a fast
    line-by-line scan (tracks the active <Network> → collects NET.STA per <Station>).
    Used by the per-config mirror to filter events to this config's stations. Cached by mtime."""
...
def _flat_station_presence(folder: Path, net: str, sta: str) -> set:
    ...
    combined in one file, so any file covering a day means all stations have data.
    """
    present: set = set()
...
def get_stations_fdsn():
    ...
    # Force text format for easy parsing
...
def _fdsn_download_worker(job_id: str, params: dict, username: str = "", password: str = "") -> None:
    ...
    Strategy (per-station-day):
      1. Fetch inventory → determine expected channels per station
      ...
      3. Download ONE request per station-day with channel=SH?,BH?,HH?,EH? + location=*
         → server returns whatever is available (no hard-coded channel/loc assumption)
      4. Split returned Stream by (loc, channel), save each trace to correct SDS path

    This way: if SH is unavailable, BH is returned automatically; if loc differs from
    inventory, the actual loc from the trace is used — no silent data loss.
    ...
    try:
        ...
        # ── Step 3: parallel download, split stream → SDS files ───────────────
...
def test_wv_client():
    ...
    # Short code (IRIS, GEOFON, …) — check via ObsPy without making network calls
...
def _xml_to_pha(xml_file: Path) -> Path:
    """Konversi QuakeML → HypoDD/VELEST phase format (.pha).
    Format output:
...
def _ql_download_worker(job_id: str, params: dict) -> None:
    ...
    try:
        ...
        # Convert XML → PHA when requested and the format is xml
...
def _xml_fast_parse_events(xml_file: Path) -> list[dict]:
    """Parse QuakeML using ElementTree (70× faster than ObsPy).
...
def _xml_to_result_events(xml_file: Path) -> list[dict]:
    """Parse QuakeML → events; uses a JSON cache when the XML hasn't changed."""
    cache = xml_file.with_suffix(".events.json")

    # Validasi cache berdasarkan mtime
    if cache.exists():
        try:
...
def get_waveform_trace():
    ...
    date_str = request.args.get("date", "")
    # Drum lays 24 hour-rows side by side, each needing ~1 bin/pixel → the full-day
    # envelope wants ~width×24 bins (tens of thousands). Cap high enough for that,
    # else rows are under-resolved and the waveform pattern is invisible.
...
def sysinfo():
    ...
    # phasenet_native workers run inside Docker → their parent is containerd-shim,
    # NOT the runner, so _resolve_session (parent-walk) can't reach the session.
    # Fall back to the job dir embedded in the predict.py cmdline (--data_list /
    # --result_dir = .../picking/<job_id>/_pn_native/...).
    import re as _re
...
def _build_pick_cfg(meta: dict, method: str, params: dict, job_id: str = "") -> dict:
    ...
    if method == "phasenet":
        # model dropdown di GUI: "original","stead","diting","instance"
        # → maps ke pretrained key di seisbench
        pretrained_map = {
            "original" : "original",
            "stead"    : "stead",
            "diting"   : "diting",
...
def _live_runner_job_ids() -> set:
    ...
    already-detached subprocesses keep running — don't incorrectly mark them 'stopped'."""
    live: set = set()
    try:
        import psutil
    except Exception:
...
def _reconcile_state(j: dict, sf, live: set, job_dir=None) -> dict:
    """Reconcile job state against actual OS processes. Live process → 'running'
    (even when _job_threads is empty after a server restart); 'running' with no
    thread AND no live process → inspect output.log first (job may have finished
    during the restart) → 'done'/'error', then fall back to 'stopped'."""
    jid = j.get("id")
    if jid in live:
        if j.get("state") != "running":
            j["state"] = "running"
...
def _kill_pick_processes(job_id: str) -> bool:
    ...
    gone) by matching the job_id in process cmdlines — fixes the 'stopped but
...
# Default reference paths point at the disertasi benchmark folders; override any
# of them with ?velest=...&hypodd=... etc. SeisWork's own relocation outputs are
# read from work/benchmark (preferred) or work/relocation.
# Dissertation-specific (Jailolo). Override via query param OR env SEISWORK_BENCH_*.
# Personal paths are only used when the file actually exists (_first_existing) —
# on another user's install without these files the benchmark is simply empty (no error).
_BENCH_BASE = os.environ.get("SEISWORK_BENCH_DIR", "/home/hakim/Disertasi_DATA/JAILOLO/PhaseNET")
BENCH_DEFAULTS = {
...
def _picks_path_or_error(job_id: str):
    ...
    Checks: job status → PermissionError → file existence."""
    job_dir = PICK_DIR / job_id
    picks_path = job_dir / "picks.csv"
...
def _denoise_worker(job_id: str, cfg: dict, cfg_id: str = "") -> None:
    """Background thread: tulis temp YAML, launch subprocess, stream log."""
    import subprocess, sys as _sys
...
def api_denoise_run():
    """Mulai job DeepDenoiser standalone."""
...
def _build_pipe_cfg(meta: dict, step: str, method: str, params: dict,
    ...
    """Build SeisWork config dict for pipeline steps (assoc, locate, …)."""
    region   = meta.get("region", {})
    waveform = meta.get("waveform", {})
    wv_path  = (waveform.get("path") or "").strip()
    ...
    if step == "assoc":
        ...
        if method == "gamma":
            ...
            # tt_grid/velocity/lat_center/tt_db → produces correct -R/-S/-G/-V
            # flags (reproducing runREAL.pl). Old keys (tt_sum/np/ns/nps/nr)
            # not read → REAL uses defaults & -G is not sent (ttdb unused).
            cfg["associate"]["real"] = {
    ...
    # initial_model. Default damping vthet=10, stathet=1 (7G_Jailolo notebook).
        ...
        cfg["velocity"] = {
            "velest": {
                ...
                # distmax velest.cmn (km): 7G_Jailolo ref = 100 (1000 → RMS increases)
                "distmax"       : float(params.get("distmax", 100.0)),
                # lat/lon reference: GUI value → midpoint of region config as fallback
                ...
                # drop events with large gap or small mag → cleaner RMS
...
def imaging_velocity_isosurface(job_id):
    ...
    range and DWS raypath coverage — 3D Slicer-inspired contour interpolation:

    1. Sparse SIMUL2000 grid (e.g. 17×17×12) → cubic RegularGridInterpolator
       → dense grid (n×n×n_dep) clipped to seismicity + station footprint.
    ...
    Query params
    -----------
    threshold  float  default 2.0  |ΔVp %| level for the anomaly surfaces
    ...
    # Final masked field: dv fades to 0 outside (DWS∩anomaly) bounded region
    # → Plotly isosurface at ±threshold stays inside ray-illuminated + anomalous volume
...
def imaging_raypaths(job_id):
    ...
    SIMUL2000-traced ray path; a visual approximation of source→station bend."""
    job_dir = PIPE_DIR / job_id
    sim_dir = job_dir / "simulout"
    if not sim_dir.is_dir():
        return jsonify({"error": "no simulout dir for this job"}), 404
...
def api_reloc_stats(job_id):
    ...
    # Box plot of residuals per station (notebook §12) — quartiles computed server-side
    # to keep it lightweight; sorted by median residual.
    box = []
...
def result_catalog(cfg_id):
    ...
    CSV is serialised vectorised (df.where → to_dict) — not iterrows which is
...
# keep only associated picks (event_index/event_id present, not -1) → a small index
# {event_id: {sta: {"net":, "phases": {PHASE: datetime}}}}, then serve clicks O(1).
...
# (velest, hypodd, …) carry NO picks file of their own — only catalogs whose
...
# arrival overlays in the Result Viewer waveform plot — QC reference like the
...
def result_waveform(cfg_id):
    ...
    rf = s.get("result_file", "")
    if not rf or not Path(rf).exists():
        return jsonify({"error": "Job has no result file"}), 404
    ...
    try:
        ...
        if ev_row.empty:
            return jsonify({"error": f"Event {event_id} not found in job {job_id}"}), 404
        ev = ev_row.iloc[0]
    except Exception as ex:
        return jsonify({"error": f"Catalog read error: {ex}"}), 500
    ...
    try:
        ...
        return jsonify({"error": f"Cannot parse event time: {ot_str}"}), 400
    ...
    # No picks of this job's own (e.g. velest/hypodd catalog) → try:
    ...
    # Predicted P/S arrivals (global IASP91 model) — QC overlay next to the
...
def slab_contours():
    ...
    both the 2-D cross-sections and the 3-D mesh surface.
    """
...
def get_elevation_profile():
    ...
    # finer grid (e.g. 16×16=256 pts) without getting an all-zero rejection.
    try:
        import time as _t
        CHUNK = 100
        elevs2 = []
...
def remote_proxy(subpath):
    """Forward /api/remote/<subpath> → remote_url/api/<subpath> server-side."""
