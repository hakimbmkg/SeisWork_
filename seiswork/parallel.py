#!/usr/bin/env python3
"""
SeisWork — Parallel chunked pipeline orchestrator
Author : HakimBMKG

Purpose
=======
Run the SeisWork pipeline over LONG time ranges (e.g. 1 year)
without OOM, maximising parallelism according to each stage's nature.

Root cause of OOM (long runs)
------------------------------
Old notebooks loaded ALL waveforms for the full year × all stations into RAM
at once (`all_streams = sds.get_waveforms(...)`) — ~600 GB on 125 GB RAM
→ immediate OOM-kill.  The SeisWork picker itself is ALREADY memory-bounded
(reads per station-day, GPU batch).  What needed fixing was the *orchestration*.

Parallel strategy (per stage)
------------------------------
  pick                 : GPU-bound, 1 GPU  → single pass over the full year,
                         memory-bounded (station-day batch).  Serial GPU,
                         I/O internally multi-threaded.
  associate/locate/mag : CPU-bound, scales per-event → PARALLEL per time-chunk
                         (default 30 days) via ProcessPoolExecutor.
  relocate (HypoDD)    : needs many nearby events (differential time)
                         → run ONCE at the end over the merged catalog.

Technical notes
---------------
* ProcessPool uses start-method "spawn": child processes are fresh, do NOT
  inherit CUDA context from the parent (which already initialised Torch during
  picking) → avoids fork-after-CUDA crash.
* The ML module has its own internal Pool; inside each chunk worker we FORCE
  `magnitude.workers = 1` to prevent nested-daemonic-pool errors.
  Throughput comes from many chunks running concurrently.
* Each chunk writes to its own directory (work/chunks/chunk_NN/...) →
  no file collisions between processes.
* Fault-tolerant: a failed/empty chunk does not stop the others.
"""
...
def _process_chunk(job: dict) -> dict:
    ...
    `job` contains only primitive/dict types (picklable for spawn):
        seiswork_root, chunk_dir, picks_file, assoc_cfg, locate_cfg,
        mag_cfg, inventory_file, label
    Returns a status dict (never raises — errors are caught internally).
    """
    t0 = time.time()
    label = job["label"]
