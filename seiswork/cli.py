...
# seiswork/cli.py lives inside the seiswork/ package dir;
# BASE_DIR is one level up = the project root (where config/, core/, work/ live)
BASE_DIR  = Path(__file__).resolve().parent.parent
WORK_DIR  = BASE_DIR / "work"
CFG_FILE  = BASE_DIR / "config" / "config.yaml"

# Prepend core/bin/ to PATH so all compiled tools are found without system install
...
def _detect_env() -> dict:
    """Figure out HOW SeisWork is installed/run on this machine — conda env, venv,
    or a bare interpreter — plus the exact command to launch it. Works on Linux
    (venv is the norm when conda is absent) and macOS/conda alike."""
    import shutil
    py = sys.executable
...
# Keep the NSImage + timer target alive for the whole process — a local that gets
# garbage-collected would leave the Dock icon blank / revert to the Python icon.
_DOCK_ICON_REFS: list = []


def _brand_native_app(name: str, icon_path: str | None = None) -> None:
    """macOS: a plain `python` process shows up as 'Python' in the menu bar
    (next to the Apple menu), in ⌘-Tab, and on Dock hover — with the generic
    Python 3.x rocket icon. Give the native GUI the SeisWork name AND Dock icon.
    No-op on other platforms / if PyObjC is unavailable.

    Two things are required for the Dock icon to actually stick when running from a
    bare interpreter (not a .app bundle):
      1. Activation policy must be Regular (0) — otherwise the process is a
         background/UIElement app and the Dock ignores setApplicationIconImage_.
      2. The icon must be RE-APPLIED after pywebview starts its run loop: pywebview
         resets the Dock icon to the interpreter's during launch, so a single
         pre-start set is overwritten. Scheduled NSTimers on the main run loop
         (which pywebview runs) re-apply it a few times over the first seconds."""
    # Menu bar / ⌘-Tab / Dock name → override the (Python framework) bundle name.
    try:
    ...
    try:
        ...
        if icon_path and os.path.exists(icon_path):
            # initWithContentsOfFile_ decodes the PNG immediately (initByReferencing
            # File_ is lazy and can read empty when the Dock first asks for it).
...
def _schedule_dock_icon_reapply(app, image) -> None:
    """Re-apply the Dock icon a few times after the run loop starts (pywebview
    overwrites it during launch). Uses NSTimers added to the main run loop, which
    pywebview runs in webview.start()."""
    try:
...
def _native_splash_html(duration_secs: float = 10.0) -> str:
    """Self-contained splash page for the pywebview window — shown instantly so
    the native window never flashes blank white while the server boots and the
    app renders. The og-preview.png is embedded as a base64 data URI so it paints
    without any network round-trip (works before the server is even up). A
    determinate progress bar fills over `duration_secs` (kept in sync with the
    minimum splash time) with a percentage and changing phase labels."""
    import base64
    img_tag = ""
    ...
    try:
        ...
        pass  # missing image → splash still shows the title + progress bar
...
def _ensure_linux_gi_typelib_path():
    ...
    default — `import gi; gi.require_version('Gtk','3.0')` raises
    "Namespace Gtk not available" even though the system WebKitGTK/GTK3
    typelibs (installed by install.sh's native-GUI step) are present, because
    GI_TYPELIB_PATH never points at them from inside the conda env. Only
    relevant on Linux; macOS uses WKWebView (no gi/GTK involved)."""
...
def _harden_webkit_native_window():
    """WebKitGTK's WebProcess defaults to GPU-accelerated (DMA-BUF) compositing.
    In VMs/containers/remote desktops where the render node isn't actually
    usable (e.g. the user account isn't in the video/render groups) that
    WebProcess crashes outright instead of falling back to software rendering
    — the native window vanishes instantly while this parent process (and the
    Flask server) keeps running, which looks like "the GUI hard-closes".
    Confirmed 2026-07-06: /dev/dri/renderD128 present but unreadable by the
    'ubuntu' user → window closed immediately on open. Force software
    compositing only when the render node isn't usable; leave GPU accel alone
    when it is. Only Linux uses WebKitGTK — macOS uses the built-in WKWebView."""
    import platform, glob
    if platform.system() != "Linux":
        return
    # The DMA-BUF renderer is the #1 cause of "the window opens then closes
    # instantly" inside VMs/containers/remote desktops — even when /dev/dri is
    # present, a VM's virtual GPU often can't actually service it and the
    # WebProcess crashes. Disabling DMA-BUF forces the compatible GLX/software
    # path; do it ALWAYS on Linux (negligible cost for a monitoring GUI, huge
    # reliability win). Escalate to full software compositing when there is no
    # usable render node at all.
    os.environ.setdefault("WEBKIT_DISABLE_DMABUF_RENDERER", "1")
    dri_ok = any(
        os.access(p, os.R_OK | os.W_OK)
        for p in glob.glob("/dev/dri/render*") + glob.glob("/dev/dri/card*")
    )
    # Inside a VM the render node can be readable yet backed by a virtual GPU
    # (QEMU/UTM, VMware, VirtualBox, …) that still crashes WebKitGTK's WebProcess.
    # Detect the VM and force full software rendering — the reliable path.
    in_vm = False
    try:
        import subprocess
...
def cmd_gui(args):
    ...
    host       = args.host
    # port default is None (not pinned) → auto-detect; an explicit --port pins it.
    port       = args.port if getattr(args, "port", None) is not None else 5000
    debug      = args.debug
    no_browser = getattr(args, "no_browser", False)
    force      = getattr(args, "force", False)

    # ── Redundancy guard: never run two servers by accident ──────────────────
    # A stale instance keeps serving OLD code (templates/static), which looks
    # like "my edits didn't apply". Detect already-running SeisWork servers and
    # either take over (--force → stop them) or refuse with a clear next step.
    ...
    if _existing and force:
        ...
        _existing = []
    if _existing:
        # SINGLE-INSTANCE: any already-running SeisWork GUI/web server blocks a
        # second launch (it would serve stale code / fight over the port). Show a
        # native popup — not just a console line — since the user likely
        # double-clicked a launcher and never sees the terminal.
    ...
    # A busy port would crash the server with "Address already in use". On macOS,
    # ports 5000 and 7000 are held by the AirPlay Receiver (ControlCenter) — it
    # replies 403, which shows up in the browser as "you don't have authorization
    # to view this page". Detect a busy port and fall forward to a free one.
    import socket, platform, time
    _bind_host = "0.0.0.0" if host in ("0.0.0.0", "") else host
    def _port_free(p: int) -> bool:
        # Bind the SAME (wildcard) host the server will use, and do NOT set
        # SO_REUSEADDR: binding 127.0.0.1 with SO_REUSEADDR succeeds even when
        # another process holds *:PORT (e.g. AirPlay on *:5000), giving a false
        # "free". A plain wildcard bind matches Werkzeug's real conflict.
    ...
    if not _port_free(port):
        # A restart (os.execv / service relaunch) can momentarily leave the previous
        # instance still holding the port. Wait for it to release BEFORE deciding it's
        # a real conflict — so a restart always comes back on the SAME port and the
        # GUI's health poll (which targets that fixed port) finds the server again.
        for _ in range(15):
            time.sleep(0.4)
            if _port_free(port):
                break
    if not _port_free(port):
        # Still busy. When the user did NOT pin a port (default), AUTO-DETECT the
        # next free one and announce it — so launching just works instead of
        # crashing on "Address already in use" (macOS AirPlay holds 5000/7000; a
        # leftover app may hold others). When a port WAS pinned explicitly, do not
        # silently drift (clients/health polls expect it) — fail clearly.
...
# A SeisWork server is a *python* process whose command line runs one of these
# entrypoints. Kept entrypoint-specific (not a bare "seiswork" substring) so a
# shell/editor/wrapper that merely has the repo path in its args is never matched.
# "seiswork gui"/"seiswork restart" (no ".py") match the installed console-script
# invocation (`.../bin/seiswork gui ...`, e.g. the systemd unit's ExecStart) —
# the ".py" variants only matched a bare `python seiswork.py gui` dev invocation,
# so the installed/systemd-managed server was never recognized by this scan.
...
def _seiswork_server_procs(extra_exclude=None):
    """Return a list of running SeisWork server/agent processes as
    [{'pid','cmd','port','kind'}]. Matches only python processes running a known
    SeisWork entrypoint, and excludes this process and its ancestor chain (the
    wrapper / `conda run` / shell) so `restart` never kills itself or an unrelated
    shell. Returns [] if psutil is not installed."""
    import re
    try:
        import psutil
    except ImportError:
    ...
    for proc in psutil.process_iter(["pid", "name", "cmdline"]):
        ...
        try:
            ...
            continue
        cmd = " ".join(argv)
        # Must be a python interpreter (excludes zsh/bash/editor wrappers whose
        # args merely contain the repo path) running a known entrypoint.
        is_python = ("python" in name.lower()
                     or (argv and "python" in os.path.basename(argv[0]).lower()))
        # (entrypoint list is gui/web/agent only — never matches `restart`/`info`)
        ...
        if "seiswork-agent" in cmd:
            ...
            # `restart` runs the GUI server in-process (its cmdline still says
            # 'restart') — classify it as a gui server so it is detected & stoppable.
            kind = "gui"
        else:
            kind = "server"
        m = re.search(r"--port[= ](\d+)", cmd)
        found.append({"pid": proc.pid, "cmd": cmd, "kind": kind,
...
# When SeisWork runs under a service manager (macOS launchd KeepAlive / Linux
# systemd), killing PIDs alone does NOT stop it — the manager immediately
# respawns the process (and each respawned `gui` then trips the single-instance
# guard → the "already open" popup the user saw). Stop/restart MUST go through the
# service manager first, then reap any stray procs (pipeline runners, mirror
# viewer, slarchive) and free the ports.
...
def _stop_seiswork_service() -> str | None:
    ...
    if kind == "launchd":
        uid = os.getuid()
        # bootout (modern) + unload (fallback) — either removes it from launchd so
        # KeepAlive stops respawning.
...
def _restart_seiswork_service() -> str | None:
    ...
    if kind == "launchd":
        uid = os.getuid()
        # Ensure it is loaded, then kickstart -k = atomic kill + restart (fresh code).
...
def _seiswork_related_procs():
    """Everything SeisWork: servers (gui/web/agent/restart/viewer) PLUS pipeline
    runners and the slarchive writing SeisWork's SDS. Self + ancestors excluded."""
...
def _stop_all_seiswork(stop_service: bool = True, quiet: bool = False) -> int:
    """Stop the service (so it can't respawn), then reap every SeisWork process
    (server, mirror, agent, runners, slarchive) and clear the portfile. Loops a
    few times to catch anything that slips through. Returns the count killed."""
    import time
    if stop_service:
...
# The GUI writes ~/.seiswork_gui.json on start so a second launch can auto-detect
# the exact host/port already serving (psutil can't read another process's listen
# socket on macOS without root). This drives both the single-instance popup and
# `seiswork open`.
    ...
    try:
        if only_pid is not None and _GUI_STATE_FILE.exists():
            ...
            if st.get("pid") != only_pid:
                return   # someone else owns it now — don't remove
...
def _proc_listen_port(pid: int) -> int | None:
    """Actual LISTEN port of a process (works when the OS allows reading its
    sockets — reliable on Linux; may be denied on macOS for cross-process)."""
    try:
        import psutil
...
def _detect_running_gui() -> dict | None:
    """Return {'pid','host','port','url'} of a live SeisWork GUI/web server, or
    None. Prefers the portfile (exact), falls back to psutil process scan."""
    import json
    # 1) Portfile — exact host/port, verify the pid is still a live SeisWork proc.
    try:
        if _GUI_STATE_FILE.exists():
            ...
            try:
                import psutil
                # Trust the portfile when its pid is a live SeisWork process. Use a
                # broad "seiswork" match (not just _SERVER_ENTRYPOINTS) so a server
                # launched via `seiswork restart` (cmdline says 'restart', not 'gui')
                # is still recognized as the running instance.
            ...
            if alive:
                ...
                _clear_gui_state()   # stale portfile → remove
    except Exception:
        pass
    # 2) psutil scan (portfile missing / older instance without one).
...
def _gui_alert(title: str, message: str) -> bool:
    """Best-effort NATIVE popup dialog (macOS osascript / Linux zenity-kdialog /
    tkinter fallback). Returns True if a dialog was shown, else prints & False.
    Set SEISWORK_NO_GUI_ALERT=1 to suppress the dialog (headless/service/tests)."""
    import platform, shutil, subprocess
        ...
        if sysname == "Linux":
            ...
            if shutil.which("kdialog"):
                subprocess.run(["kdialog", "--title", title, "--sorry", message],
                               timeout=60)
                return True
        # Windows or Linux without zenity/kdialog → tkinter (usually bundled).
...
def _run_native_window(host: str, port: int, start_server: bool = True) -> bool:
    """Open the SeisWork GUI in a native desktop window (pywebview / WKWebView /
    WebKitGTK). start_server=True boots the Flask server on a background thread
    first; start_server=False attaches to an ALREADY-running server (used by
    `seiswork open`). Returns False when pywebview is unavailable so the caller
    can fall back to the browser. Blocks until the window is closed."""
    # A headless machine (SSH into a server VM, no DISPLAY/Wayland) can't show a
    # native window — don't even try; the caller falls back to the browser/URL.
    ...
    # `seiswork` can resolve as a namespace package under some editable installs
    # (submodules import fine but __init__.py attributes like __version__ don't) —
    # so read the version defensively instead of `from seiswork import __version__`.
    try:
    ...
    # Same flow on Linux (WebKitGTK) and macOS (WKWebView): _harden_webkit_native_
    # window() above already forces software rendering / disables DMA-BUF when a
    # VM or unusable render node is detected, which is what actually crashed the
    # WebProcess — the splash's transparency was never the cause, so keep it.
    ...
    def _reveal():
        if state["shown"]:
            return
        state["shown"] = True
        # pywebview (6.2.1) never clears Window.hidden after construction, and its
        # GTK backend re-hides the window on every show() call while that flag is
        # still true (create_window() itself calls show() once during setup) — so
        # a window created with hidden=True stays permanently unmapped no matter
        # how many times .show() is called afterward. Clearing it here before the
        # real show() is the only way this "main" window (created hidden so it
        # doesn't flash before the splash) ever becomes visible.
        main.hidden = False
        try:    main.show()
    ...
    # Native menu bar (Help / Changelog) — the web GUI puts these in the in-page
    # navbar instead (there's no OS window chrome to hang a menu off of there).
    # Best-effort: older pywebview lacks webview.menu, and some backends ignore
    # a menu bar entirely — either way the app must still start without one.
    native_menu = []
    try:
        ...
        def _open_help():
            try:
                webview.create_window("SeisWork — Help", url=local_url + "/help",
        ...
        def _open_changelog():
            try:
                main.evaluate_js("SW.openChangelog()")
            except Exception:
                pass

        native_menu = [Menu("Help", [
            MenuAction("Documentation", _open_help),
            MenuAction("Changelog", _open_changelog),
        ])]
    except Exception:
        pass

    # On Linux (WebKitGTK) the window/taskbar icon comes from the `icon` argument
    # to webview.start — the macOS Dock-branding above (Foundation/AppKit) is a
    # no-op there. Passing it is harmless on macOS. Older pywebview lacks the
    # kwarg, so fall back gracefully.
    # private_mode=False: pywebview defaults to an ephemeral/private session, and
    # in WebKitGTK that makes `window.localStorage` outright null (not just empty)
    # — the web app's own JS (SeisWorkCore's constructor) reads localStorage
    # unconditionally, so the native window loaded a blank page stuck on "Loading
    # interface..." with a TypeError on every boot. A real desktop window should
    # persist local storage like a normal browser tab anyway.
    try:
        webview.start(_boot, icon=(_icon_png if os.path.exists(_icon_png) else None),
                      private_mode=False, menu=native_menu)
    except TypeError:
        webview.start(_boot, private_mode=False)   # pywebview without the icon/menu kwargs
    return True


def cmd_stop(args):
    """Stop EVERYTHING: the service (launchd/systemd, so it can't respawn), the
    GUI + mirror + agent servers, pipeline runners, and slarchive — freeing all
    ports. SeisWork stays down until started again."""
    try:
...
def cmd_restart(args):
    """Hard-restart SeisWork so edited code/assets actually reload. Truly kills all
    pipelines, PIDs and ports first. When installed as a service (launchd/systemd)
    it restarts THE SERVICE atomically (kill + relaunch fresh) — killing PIDs alone
    can't work because the service respawns them. Pass --stop to only stop;
    --foreground to force a foreground (native) relaunch instead of the service."""
    try:
        import psutil  # noqa: F401
    except ImportError:
    ...
    if svc and not foreground:
        # Service-managed → restart the service cleanly (fresh code, KeepAlive
        # intact). First reap stray runners/mirror/slarchive so nothing lingers.
...
def cmd_open(args):
    """Open ONLY the GUI window against an ALREADY-running SeisWork server (no new
    server). If none is running, guide the user to start one."""
    running = _detect_running_gui()
    if not running:
    ...
    if _run_native_window("127.0.0.1", port, start_server=False):
        return
    # Native unavailable/headless → open a local browser if there's a display,
    # otherwise just print the URL(s) the user can open from their own machine.
    import platform as _plat
