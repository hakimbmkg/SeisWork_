...
class VelestVelocity:
    ...
    def _build_phase_file(self, catalog_csv: str, picks_csv: str,
        ...
        Picks are matched to events using a time window (most robust across
        association methods: GaMMA, REAL, etc.).
        """
        ...
        # event_index = -1 were never associated). Keep only associated picks
        # so the catalog<->pick join stays small and fast.
        ...
        for _, ev in cat_df.iterrows():
            ...
            # Keep only the shortest travel-time per (station, phase) to
            # avoid PHASETEST duplicate-pick errors in VELEST.
    ...
    def _write_cmn(self, neqs: int, mode: int) -> str:
        ...
        distmax  = self.vcfg.get("distmax",  100.0)   # ref 7G_Jailolo (1000 raises RMS)
    ...
    # ── Parse final.CNV into catalog_velest.csv ──────────────────────────────
    def _parse_final_cnv(self, cnv_path: str, out_csv: str) -> int:
        ...
        Parse VELEST final.CNV format (fixed-field, space-separated):
          ...
          MMDD: 1-4 digits (812 = M08,D12 | 1012 = M10,D12)
          HHMM: 1-4 digits (941 = H09,M41 | 0941 = H09,M41)
        """
        from datetime import datetime as _dt
        rows = []
