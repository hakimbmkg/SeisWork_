...
class STALTAPicker:
    ...
    def run(self):
        ...
        # Worker count: each SDS/file worker loads full-day waveform (~70 MB).
        # Cap at 4 to avoid OOM on typical 8 GB machines (4 × 70 MB << 2 GB).
        ...
        if self.is_sds:
            ...
            for key, st in self._iter_sds_streams():
                ...
                if not st:
                    continue
                tr0 = st[0]
                # use channel from the actual stream (already auto-detected in _iter_sds_streams)
        ...
        # Output filename: picks.csv (consistent with PhaseNet/EQT for GUI jobs)
