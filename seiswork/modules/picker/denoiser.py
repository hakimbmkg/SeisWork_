...
def denoise_stream(st, pretrained: str = "original", batch_size: int = 1):
    ...
    # annotate() returns traces named NET.STA.LOC.CHA__DeepDenoiser_signal;
    # keep only the signal traces and restore the original channel names.
    signal_traces = []
    ...
    if not signal_traces:
        # Fallback: no signal component returned — use the original trace.
