...
Model weights are auto-downloaded by seisbench on first use (~50 MB).
...
─── Data sources (pick.phasenet.data_source) ───────────────────────────────
  ...
  "sds_lite" — read directly from SDS, but LIGHTLY,
               scan record headers with `mseedlite` (pure-Python, no
               compiled libmseed, no STEIM decode, no _MSEED_LOCK needed),
               keep only raw bytes of records overlapping the requested
               window, decode ONLY that filtered subset via ObsPy.
  "fdsn"     — download from FDSN server → save to SDS → pick
...
# ════════════════════════════════════════════════════════════════════════════
#  Old picker: 1 process, ThreadPool I/O serialised by _MSEED_LOCK → MSEED
#  read+parse SERIAL on 1 core → GPU idle 0%.  This mode runs N reader PROCESSES:
#  each process has its own libmseed (NO lock, parallel parse across many cores)
#  → fetch+merge+filter+slice per station-day, then sends annotation-ready
#  chunks to the main process running the GPU.  I/O (processes) OVERLAPS GPU (main).
#
_MP = {}   # per-process reader globals (set by _mp_init)


def _measure_amp(raw_stream, t_pick, phase):
    """Peak-to-peak half-amplitude (counts) on the raw/filtered waveform at pick time.

    Window: P → Z channel, [-0.5, +2.0] s around pick;
            S → N/E channel, [-0.5, +3.0] s.
    Returns float or nan on any failure (called from thread pool, must not raise).
    """
    import numpy as np
...
def _mp_read_prep(task):
    ...
    Returns (key, [chunk_Stream, ...]) or (key, None) if empty/failed.
    Does NOT touch torch/CUDA.  Does NOT use _MSEED_LOCK (process is isolated).
    """
...
def _gpu_shard_worker(cfg, base_dir, shard_tasks, shard_id):
    """Runs IN A WORKER PROCESS (spawn ctx): pick one shard of the task list on
    the shared GPU and write picks_shard_<id>.csv. Each worker loads its OWN model
    (spawn → no inherited CUDA context from the parent). Adapts the multi-process
    GPU saturation of /home/hakim/datas/PhaseNet_GPU.sh to seisbench/PyTorch."""
    try:
...
class PhaseNetPicker:
    ...
    def __init__(self, cfg: dict, base_dir: str):
        ...
        # picks_dir: GUI passes job-specific path (PICK_DIR/<job_id>/);
        # CLI/notebook not set → default work/picks/
        ...
        # io_processes.
        # 0 = legacy path (ThreadPool, lock-serial).  >0 = N parallel reader processes
        # feeding the GPU (overlapping I/O↔GPU).  Only applies to data_source "sds".
        self.io_processes = int(self.pcfg.get("io_processes", 0))

        # gpu_workers (adapted from /home/hakim/datas/PhaseNet_GPU.sh): N full processes
        # in parallel on the same GPU.  One PhaseNet model does not saturate H100 →
        # split task list (station-day / mseed file) into N shards, each worker
        # has its own model (PyTorch shares GPU), runs picks serially on
        # its shard → writes picks_shard_<id>.csv, then parent merges.  1 = off.
        self.gpu_workers = int(self.pcfg.get("gpu_workers", 1))
        # internal worker hooks (set when running as a shard worker):
        self._tasks_override = None   # use this task list (instead of re-collecting)
        self._shard_id       = None   # shard id → output name picks_shard_<id>.csv

        # SDS config
        self.sds_path    = Path(self.pcfg.get("sds_path") or str(self.wave_dir))
        self.sds_chunk_s    = int(self.pcfg.get("sds_chunk_days",    1)) * 86400
        # annotate_chunk_hours: slice each stream into N-hour sub-chunks before
        # GPU annotation.  Prevents seisbench from allocating a single huge
        # tensor for a full day of data.  Default 1 h = 360 000 samples at 100 Hz.
    ...
    def _fetch_sds_lite(self, task: tuple):
        """Lightweight SDS fetch (see seiswork.utils.sds_lite).

        Scans record headers with `mseedlite` (pure-Python, lock-free) and
        decodes ONLY the byte range overlapping the task window — instead of
        decoding a full day-file then discarding what's outside the window.
        No `_MSEED_LOCK` needed for the scan; the (much smaller) ObsPy decode
        step is serialised internally by `sds_lite._DECODE_LOCK`.
        """
        from obspy import UTCDateTime
        net, sta, loc, chan, t0, t1 = task
        t0u, t1u = UTCDateTime(t0), UTCDateTime(t1)
    ...
    def _lite_reader(self):
        """Lazily build & cache the SDSLiteReader instance for this picker."""
    ...
    def _write_to_sds(self, st, sds_write_path: Path) -> int:
        """
        Write stream to SDS archive (split by calendar day).
        Path: {root}/{year}/{net}/{sta}/{chan}.D/{net}.{sta}.{loc}.{chan}.D.{year}.{doy}
        Appends to existing file if present.  Returns number of files written.
        """
    ...
    def _slice_chunks(self, st):
        """
        Yield ann_chunk_s-second sub-streams.
        Prevents seisbench from allocating a single huge tensor for a full day.
        """
        t     = min(tr.stats.starttime for tr in st)
        t_end = max(tr.stats.endtime   for tr in st)
        while t < t_end:
    ...
    # Minimum time gap (seconds) between picks of the same phase at the same station.
    # Prevents S-coda oscillations near threshold from generating many near-duplicate picks.
    _NMS_GAP_S = 2.0
    ...
    def _run_gpu_parallel(self, n_gpu: int, n_w: int):
        """Adapt PhaseNet_GPU.sh: build the task list (smart list of station-day /
        mseed-file jobs), split it into N shards, run N worker processes that each
        pick their shard on the SAME GPU (own model each → saturates the H100),
        then merge picks_shard_*.csv → picks.csv."""
        ...
        # Round-robin split → each shard mixes stations/days (balanced load).
        # The split is deterministic (_collect_*_tasks scans sorted dirs), so a
        # given n_gpu always reproduces the same shard i → picks_shard_<i>.csv.
        shards = [tasks[i::n_gpu] for i in range(n_gpu)]

        # ── RESUME: skip shards already completed by a previous run ──────────
        # picks_dir is job-specific, so any picks_shard_<i>.csv present here was
        # produced by an earlier (partial) run of THIS same job. Re-running after
        # an OOM-killed worker only re-picks the missing shard(s) instead of
        # throwing away the hours already spent on the survivors. A successful
        # merge deletes the shard files (below), so a fresh job finds none.
        def _shard_done(i):
        ...
        # Any worker we launched MUST write picks_shard_<id>.csv. If it didn't,
        # its station-days are unpicked and merging the survivors would silently
        # DROP them. This catches BOTH failure modes:
        #   • OOM-killed (SIGKILL) → exitcode -9, no output
        #   • caught exception inside _gpu_shard_worker (e.g. broken torch import)
        #     → the worker prints a traceback but returns normally → exitcode 0
        # The earlier "exitcode != 0" guard missed the second case and merged a
        # partial catalog. Trust the artifact, not the exit code: no file = fail.
        failed = []
    ...
    def _run_mp(self, n_proc: int):
        """N reader processes (fetch+merge+filter+slice) → GPU in the main process.

        Replaces the legacy path (ThreadPool lock-serial) for data_source "sds".
        Pool uses 'spawn': child processes are fresh, do not inherit CUDA from parent → safe.
        Readers do not import torch.  imap_unordered overlaps I/O (workers) with
        GPU (main); GPU is fast so the result buffer stays small (memory-bounded).
        """
        import multiprocessing as mp
        import torch
            ...
            for key, chunks in pool.imap_unordered(_mp_read_prep, tasks,
                ...
                if done % 10 == 0 or done == len(tasks):
                    ...
                    sys.stdout.write(
                        ...
                        f"empty={cnt['empty']} GPU={cnt['annotated']} "
    ...
    def run(self, workers: int = None):
        """
        Architecture: serial batches to avoid fork-after-thread crash.

        On Linux, seisbench annotate() may fork DataLoader workers.  If a
        ThreadPoolExecutor is active at fork time, the child inherits a
        dead-locked mutex → SIGSEGV → Jupyter kernel crash.

        Fix: each phase (I/O, GPU, extract) runs to completion before the
        next begins.  Memory is bounded by io_batch × stream_size.

        For N tasks, io_batch = max(n_workers×4, 32):
          Batch 1: [read io_batch streams] → [GPU annotate all] → [extract all]
          Batch 2: ...
        """
        import torch
        ...
        # ── Parallel GPU workers (PhaseNet_GPU.sh-style): split the task list
        # across N full worker processes on the same GPU, then merge. ──────────
        if self.gpu_workers > 1 and self._shard_id is None:
            return self._run_gpu_parallel(self.gpu_workers, n_w)

        # ── Option A: multi-process SDS reader (overlap I/O↔GPU) ──────────────
        ...
        for b_idx in range(0, len(tasks), io_batch):
            ...
            with ThreadPoolExecutor(max_workers=n_w) as pool:
                ...
                for fut in as_completed(futs):
                    ...
                    sys.stdout.flush()
            # ThreadPoolExecutor has exited — no threads active → GPU safe
            ...
            # annotations: (key, ann_stream, filtered_stream_ref)
            # filtered_stream_ref kept alive here so amplitude can be
            # measured in Step 3 even after streams.clear() removes the dict.
            annotations = []
            for key, st_raw in streams.items():
                try:
                    ...
                    cnt["ann_err"] += 1
                sys.stdout.write(
                    f"\r  {b_label} GPU {cnt['annotated']} chunks"
                    f"  err={cnt['ann_err']}"
                )
                sys.stdout.flush()
