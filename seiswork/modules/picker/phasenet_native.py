...
Drives the original AI4EPS PhaseNet (TensorFlow `predict.py`) directly instead of
the seisbench port. This adopts /home/hakim/datas/PhaseNet_GPU.sh into SeisWork as
a first-class picker backend:

    SDS scan ─► fname.csv (per-station-day globs) ─► split into N shards
             ─► N × predict.py worker subprocess (saturate the GPU)
             ─► convert + merge native picks.csv → SeisWork schema → picks.csv

Why native over the seisbench path
----------------------------------
  • RAM-light. Each worker streams ONE station-day at a time (~200 MB) instead of
    buffering dozens of day-streams in a serial batch → no OOM on a 29 GB box
    (the failure that killed the full-year seisbench run).
  • GPU-saturating. N independent TF processes (gpu_options.allow_growth) share
    the H100; one PhaseNet model never fills it. NUM_WORKERS-style parallelism,
    exactly like PhaseNet_GPU.sh.
  • Zero data copy. fname entries are SDS *globs* (`.../HH?.D/...`); obspy.read
    expands the three components straight from the archive — no temp mseed.
  • Fast. ~7.5 s/station-day on the H100 → ~19 000 station-days ≈ 4 h at 10
    workers (vs ~9–13 h for the seisbench path).

Resume
------
Each shard writes out_<i>/picks.csv (+ an out_<i>/.done marker on clean exit).
Re-running the same job skips shards already marked done and only re-picks the
missing ones, so an interrupted run is cheap to finish.
"""
...
# Default location of the AI4EPS PhaseNet repo + model (cloned by install.sh).
# Overridable via pick.phasenet_native.{phasenet_dir,model_dir,docker_image} OR the
# PHASENET_DIR / PHASENET_MODEL envs. Default ~/apps/PhaseNet (portable, not a
# specific user's personal path).
...
class PhaseNetNativePicker:
    ...
    def __init__(self, cfg: dict, base_dir: str):
        ...
        # Execution engine: Docker ONLY. predict.py runs inside the TF-GPU image
        # (seiswork/phasenet:tf2.12, Hopper-compatible cuDNN 8.6), fully isolated
        # from the host conda envs. No conda fallback — the host only needs Docker
        # (+ nvidia-container-toolkit for GPU); the runner is launched via
        # `sg docker -c ...` so it has the `docker` group. Built by install.sh.
        self.docker_image = self.pcfg.get("docker_image", "seiswork/phasenet:tf2.12")

        # process label for system monitor: job id = job-specific picks_dir name
        self.job_label = self.picks_dir.name or "picks"
        # seconds to wait between launching successive GPU workers, so their
        # first big cuDNN conv doesn't spike GPU workspace memory simultaneously
        self.stagger_s = float(self.pcfg.get("stagger_s", 10))
        # skip SDS day files larger than this (corrupt >24h files → giant tensor
        # → cuDNN crash). 0 disables. Default 50 MB (normal 200 Hz day ~38 MB).
    ...
    def _load_allowed_stations(self) -> set:
        """(network, station) pairs the picking is RESTRICTED to — from the
        config's station_file (the GUI writes the selected stations as
        'net|sta|lat|lon|elev'). Empty set = no restriction (pick every station
        in the SDS archive). Station-only rows (no network) match any network
        via the ('*', sta) sentinel."""
    ...
    def _collect_station_days(self) -> list:
        """Scan SDS → one (glob, size_bytes) per (station, day) that has a
        vertical-channel file within the region time range. The glob
        (`.../<band>?.D/...<band>?...`) expands to the 3 components when
        obspy.read()s it inside predict.py. size_bytes (the Z-file size, a proxy
        for the 3-component data volume) is used to balance the shards so every
        GPU worker gets ~equal load and they finish together."""
        ...
        # Restrict to the config's selected stations (saves time — don't pick the
        # whole SDS archive). Empty = no restriction.
                ...
                for sta_dir in sorted(net_dir.iterdir()):
                    ...
                    for f in sorted(z_dir.iterdir()):
                        ...
                        try:
                            ...
                            size = 0
                        # Skip corrupt SDS day files that hold MUCH more than 24 h
                        # (e.g. overlapping/merged days → >34 M samples). Resampled
                        # to 100 Hz they become a ~17 M-sample tensor that blows up
                        # cuDNN on Hopper (CUDNN_STATUS_EXECUTION_FAILED) and kills
                        # the whole shard. A normal 200 Hz full day is ~38 MB.
    ...
    def _balance_shards(tasks: list, n: int) -> list:
        """Greedy longest-processing-time bin-packing: split (glob, size) tasks
        into n shards with ~equal TOTAL bytes, so all GPU workers finish around
        the same time (vs round-robin, which only balances COUNT — uneven when
        station-days differ in length/coverage)."""
    ...
    def run(self, workers: int = None):
        ...
        # Balance shards by DATA VOLUME (not just count) so all GPU workers
        # finish together — see _balance_shards.
        ...
        for k, i in enumerate(todo):
            ...
            # Stagger startups: if all workers fire their first (large) conv at
            # once, the cuDNN workspace allocations spike together and one worker
            # dies with CUDNN_STATUS_EXECUTION_FAILED. A short delay lets each
            # worker reach bounded steady-state memory before the next starts.
            if k > 0:
        ...
        for i, pr in procs.items():
            ...
            out_dir = self.work_dir / f"out_{i}"
            if rc == 0:
                # predict.py only writes picks.csv when picks>0; mark done either way
                (out_dir / ".done").touch()
        ...
        # ── auto-retry failed shards SEQUENTIALLY (alone = full GPU) ─────────
        # A shard can die with CUDNN_STATUS_EXECUTION_FAILED when several big
        # convs contend for GPU workspace at once — typically the single largest
        # station-day, which volume-balancing front-loads. Re-running each failed
        # shard ALONE (all other workers already joined) gives it the whole GPU,
        # so even the biggest day fits. Only shards that fail even solo are fatal.
        if failed:
            retry, failed = failed, []
    ...
    def _label_process(self):
        """Label this runner PID in `top`/`htop`/sys-monitor with the picker
        method (no extra deps): set the kernel thread name via /proc/self/comm
        (max 15 chars). Workers are labelled separately via docker --name."""
        try:
    ...
    def _progress_monitor(self, todo, shards, stop_event, interval=25):
        """Daemon thread: workers write tqdm to their own out_<i>/worker.log, so
        the runner's stdout (what the GUI streams) stays silent. Aggregate every
        shard's latest progress and print a line the GUI recognizes
        ('read=N/TOTAL') → live progress bar in the pick terminal."""
    ...
    def _predict_args(self, fname_csv, result_dir) -> list:
        """predict.py CLI args (paths are identical inside the docker container,
        since SDS/PhaseNet/picks_dir are bind-mounted at their host paths)."""
        args = [
    ...
    def _launch_worker(self, idx: int, fname_csv: Path, out_dir: Path):
        """Spawn one predict.py worker on a shard inside the TF-GPU container
        (Docker only). Mirrors PhaseNet_GPU.sh. Bind-mounts SDS + PhaseNet (ro)
        and picks_dir (rw) at IDENTICAL host paths so all absolute paths (globs,
        model, result_dir) resolve unchanged inside the container. --user keeps
        outputs owned by the host user (no root-owned files)."""
        log_fh = open(out_dir / "worker.log", "w")
        args   = self._predict_args(fname_csv, out_dir)
        uid, gid = os.getuid(), os.getgid()
        # Persistent CUDA JIT cache (HOME/.nv/ComputeCache). TF 2.12 has no sm_90
        # cubins → it JIT-compiles kernels from PTX on first use (~34 s). A shared
        # persistent cache lets later workers / resumes / re-runs reuse compiled
        # kernels instead of re-JITing each time.
        cache_dir = self.work_dir / "nv_cache"
        cache_dir.mkdir(parents=True, exist_ok=True)
        # container name = label in `docker ps` AND in host `ps`/sys-monitor
        # cmdline → instantly shows method+job+shard for any PID.
    ...
    def _merge_and_convert(self, n_w: int):
        ...
        for i in range(n_w):
            ...
            if not f.exists():
                continue          # shard produced zero picks → no file (ok)
            try:
                d = pd.read_csv(f)
            except Exception:
                continue
