...
class GammaAssociator:
    ...
    def _load_sensitivities(self) -> dict:
        """Map NET.STA -> sensitivity (counts/(m/s)) from StationXML.

        Picker measures phase_amp in raw counts, but GaMMA expects ground
        velocity in m/s. Feeding counts straight in pushes magnitude to the
        +8 ceiling, so we divide by station sensitivity first. Sensitivity
        varies a lot across the network (broadband vs short-period), so we
        need a per-station map, not one global value.
        """
    ...
    def _amp_counts_to_velocity(self, picks_df: pd.DataFrame) -> pd.DataFrame:
        """Convert phase_amp from raw counts to ground velocity (m/s).

        Stations missing from the inventory fall back to the network-median
        sensitivity. No-op if inventory is unavailable (caller warns that
        magnitudes will be off).
        """
    ...
    def _load_stations(self) -> pd.DataFrame:
        ...
        # Drop extra columns (e.g. "location", often all-NaN). GaMMA's
        # convert_picks_csv() drops rows with any NaN, so a stray all-NaN
        # column would wipe out every pick.
    ...
    def _build_gamma_config(self) -> dict:
        ...
        # GaMMA API v0.0.1+ requires these keys; old keys (xlim_km, ylim_km,
        # zlim_km, min_picks_per_event, max_sigma) are obsolete.
        sigma = self.gcfg.get("max_sigma", 2.0)

        # DBSCAN pre-clustering keys (only used when use_dbscan=True; missing
        # them raises KeyError: 'dbscan_eps'). eps defaults to network
        # aperture / P velocity so picks from opposite ends of a sparse,
        # wide network still cluster together (a small eps tuned for dense
        # networks would leave everything unclustered).
    ...
    def _to_standard(self, events_list) -> pd.DataFrame:
        # association() returns list[dict], not a DataFrame. Event center is
        # in projected UTM coords (x/y/z(km)), so it must be inverse-projected
        # back to lat/lon - GaMMA never sets "lat"/"lon"/"latitude" directly.
        rows = []
    ...
    def run(self, picks_file: str):
        ...
        # Convert phase_amp counts -> m/s before association, so GaMMA's
        # built-in magnitude is physical, not pinned to the +8 ceiling.
        # No-op if no inventory.
        ...
        if gcfg["use_amplitude"] and ("amp" not in picks_df.columns or picks_df["amp"].isna().all()):
            ...
            # Partial NaN (channel missing during amplitude measurement):
            # drop those picks so BGMM doesn't crash on "Input X contains NaN".
        ...
        # Build id as NET.STA for station matching. station_id in picks.csv
        # includes the seisbench channel suffix (e.g. "7G.SP25..Pha"), which
        # doesn't match station df id ("7G.SP25"), so prefer network+station.
        ...
        # Merge assignments back. `assignments` is a sparse list of
        # (pick_idx, event_index, prob), one entry per pick that got assigned
        # (not one per input pick). Unassociated picks keep event_index=-1.
        ...
        # names used by downstream steps (NLLocLocator._catalog_to_obs etc.)
