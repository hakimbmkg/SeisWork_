...
class PyOctoAssociator:
    ...
    # associate() needs "station" (unique site id, matches station df "id"),
    # "phase" (P/S), and "time" (float Unix epoch seconds, not a Timestamp -
    # associator.py:544 requires a float). Other columns (network, phase_score,
    # phase_amp, ...) pass through associate() unchanged and come back merged
    # into `assignments` (associator.py:709-712 merges on "idx"), so we keep
    # them here instead of re-joining later.
    ...
    # transform_stations() needs "id" (matches picks["station"]), "latitude",
    # "longitude", "elevation" (meters above sea level - associator.py:950-952
    # flips sign internally to get z in km below zero).
    ...
    def run(self, picks_file: str):
        ...
        # raise inside the C++ backend when looking up coordinates.
