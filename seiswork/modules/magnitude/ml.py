...
ML = log10(A_WA_mm) + a·log10(dist_km) + c·dist_km + b
  where A_WA_mm = peak Wood-Anderson amplitude in mm,
  a,b,c = regional attenuation coefficients (Hutton & Boore 1987, BSSA 77,
  pp. 2074-2094, original distance-correction term — verified 2026-06-22
  against the published formula + REAL.c's own built-in magnitude, both of
  which independently use r0=100/intercept=3.0 — NOT the r0=17/intercept=2.09
  that was here before, which didn't match any published source):
  a=1.110, c=0.00189, b=0.591, equivalent to
  1.110*log10(R/100) + 0.00189*(R-100) + 3.0.

Cross-checked 2026-07-04 against SeisComP's own reference implementation
(libs/seiscomp/processing/magnitudes/{ML,MLv}.cpp and amplitudes/MLv.cpp) —
three deviations found and fixed there:
  1. All SeisWork stations report a single vertical channel (BHZ) only, which
     SeisComP treats as "MLv" (not "ML") and empirically doubles the measured
     amplitude before applying the (horizontal-calibrated) ML formula — see
     amplitudes/MLv.cpp: `amplitude->value *= 2.0`. Missing this made every
     SeisWork magnitude ~log10(2) = 0.30 units too small. See VERTICAL_AMP_FACTOR.
  2. SeisComP's amplitude measurement window scales with distance
     (`setSignalEnd("min(R/3 + 30, 150)")`, R in km) so the S-wave/coda is
     still inside the window at regional distance. SeisWork used a FIXED
     +60 s window from origin time, which is shorter than the S-wave travel
     time itself beyond ~150 km — silently measuring the (much smaller) early
     P-wave amplitude instead of the true peak. See _signal_window_end().
  3. SeisComP's MLv has effectively no useful distance/depth cutoff
     (maxDist=8°, ~890 km; maxDepth=1000 km). SeisWork's dist_max was 300 km
     (hypocentral) — far tighter, so for a sparse/nationwide network many
     events had no station within range and got no magnitude at all. Raised
     the default to 700 km (upper edge of Hutton & Boore's validated range).
"""
...
def _signal_window_end(dist_km: float) -> float:
    ...
    Mirrors SeisComP's own default (`setSignalEnd("min(R/3 + 30, 150)")`,
    amplitudeprocessor.cpp) so the S-wave/coda — not just the earlier, much
    smaller P-wave — falls inside the measured window at regional distance.
    """
...
def scmag_trimmed_mean(values, percent: float = 25.0):
    ...
    (Math::Statistics::computeTrimmedMean(), src/base/common/libs/seiscomp/
    math/mean.cpp:130, driven by magtool.cpp:1301 MagTool::computeAverage —
    case Default: trim 25% (12.5% each tail) when more than 3 station
    magnitudes are available, else fall back to a plain mean).

    Sorts the values, gives full weight (1.0) to everything strictly inside
    the trimmed range, zero weight to everything strictly outside, and a
    FRACTIONAL weight to the single boundary element on each tail (handles
    a trim count that isn't a whole number of stations) — then returns the
    weighted mean and weighted (sample) standard deviation.

    :param values: station magnitude values (any order)
    :param percent: total percent trimmed across BOTH tails combined
                    (25 => 12.5% off each end) — 0 reduces to a plain mean.
    :return: (value, stdev) — stdev is 0.0 when fewer than 2 effective
             (non-zero-weight) values contribute.
    """
    n = len(values)
    if n == 0:
        return None, 0.0
...
# _compute_ml_event is independent per event, so events can be farmed out to a
...
# task would eat the gains. Kept at module level (not bound methods) so the
# initializer/task callables pickle cleanly under both fork and spawn.
_worker_state = {}
...
class MLMagnitude:
    ...
    # see GammaAssociator / NLLocLocator) — lets notebooks redirect output to a
    # per-run working dir.
    @property
    ...
    def _init_sds_client(wave_dir):
        ...
        (YEAR/NET/STA/CHAN.D/...). The client constructs the exact file path
        per request — O(1) — instead of the recursive glob.glob() that
        _find_waveform falls back to, which walks the WHOLE archive tree
        (tens of thousands of files) on every single station/event lookup
        and dominates _compute_ml_event runtime. Returns None for non-SDS
        waveform_dir layouts (e.g. a flat directory of .mseed files), so the
        glob-based fallback still works for those configurations.
        """
        try:
    ...
    def _load_stations(self) -> dict:
        ...
        # the older NET|STA|LAT|LON|ELEV. A fixed usecols=[0,1,2,3,4] misreads
        # the empty LOC field as "lat" and shifts lat/lon by one column —
    ...
    def _compute_ml_event(self, ev: dict, stations: dict, inventory) -> float:
        ...
        for sta, (net, slat, slon) in stations.items():
            ...
            try:
                ...
                # Deconvolve true instrument response to displacement, THEN
                # convolve with the Wood-Anderson PAZ — both steps are required
                # to get a genuine simulated-WA seismogram (the amplitude this
                # function reports as "Wood-Anderson amplitude"). Skipping the
                # simulate() step leaves the trace in raw ground displacement
                # (~1e-7 m), ~1e4x smaller than a WA trace, throwing ML off by
                # ~-5 units.
                # Response removal is REQUIRED: without it the trace stays in raw
                # counts (≈ counts × WA gain ~2080) and ML over-estimates wildly
                # (e.g. M12). If the response cannot be removed (no inventory, or
                # the loc-code / epoch does not match the metadata), SKIP this
                # station rather than measuring ML from raw counts.
                if not inventory:
                    continue
                try:
                ...
                # scales with distance (win_end, see _signal_window_end) so the
                # S-wave/coda — not just the earlier, smaller P-wave — is
                # captured at regional distance.
                for tr in st:
                    ...
                    # see amplitudes/MLv.cpp. All SeisWork stations are BHZ-only.
    ...
    def _station_channel(self, inventory, network: str, station: str) -> str:
        ...
        Looks up the real channel from the StationXML inventory (any band code —
        BH/HH/SH/EH/... — not just "HH"), preferring a vertical ("...Z")
        channel since that's what the WA/ML pipeline measures. Falls back to
        the broad wildcard "?H?" (any high-gain seismometer band) when the
        inventory has no entry for this station, so waveform lookup still
        works instead of silently missing non-HH networks.
        Cached per (network, station) — the inventory is static for a run.
        """
    ...
    def _get_stream(self, network: str, station: str, t0, win_end: float = 70.0,
        ...
        code when known, else the broad "?H?" wildcard — not hardcoded "HH?",
        which silently misses any non-HH network (e.g. BHZ).

        Prefers the SDS client (direct path construction — see
        _init_sds_client) which is ~6x faster per lookup than the recursive
        glob fallback AND fetches all three components in one call (no
        separate read()). Falls back to _find_waveform()+read() for
        non-SDS waveform_dir layouts or if the SDS lookup misses
        (e.g. gaps in the archive — see project_sds_waveform memory).
        """
    ...
    def run(self, catalog_file: str, inventory_file: str = ""):
        ...
        # Fall back to the inventory path declared in cfg["magnitude"]/cfg["data"]
        # when the caller doesn't pass one explicitly (see notebooks/mag_cfg for
        # the config schema) — response removal needs this inventory, otherwise
        # ML comes out ~10 units too high.
