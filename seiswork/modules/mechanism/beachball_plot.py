"""
SeisWork — Focal mechanism beachball plot (red/white, pseudo-3D shaded sphere,
transparent background) — by HakimBMKG
...
SKHASH's own plot_dc()/aux_plane() — the projection math, not reimplemented)
rendered as a shaded sphere: the focal sphere really is a sphere, so a
diffuse-lighting shade computed analytically from each pixel's position on
the unit sphere (normal == position, since centered at origin) gives a
genuine 3D appearance from a single 2D image, with the background pixels set
fully transparent (alpha=0) instead of plain white.
"""
