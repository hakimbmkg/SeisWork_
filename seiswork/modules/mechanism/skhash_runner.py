...
SKHASH itself only consumes: an event catalog, P-polarity picks, a station
...
SKHASH's own main() calls sys.exit() at the end — it MUST be invoked as a
...
DEFAULT_CONTROL = {
    ...
    # near/outside the network footprint) — azimuthal gaps of 100-300 deg are
    # common, so the typical dense-network default (90) rejects everything.
...
class FocalMechanism:
    ...
    def run(self, catalog_csv, picks_csv, sds_path, inventory_path,
        ...
        # outfolder_plots intentionally left blank — SKHASH's own beachball
        ...
        # out.csv + out_polinfo.csv this run produces — no need to ask SKHASH
        # to plot at all.
