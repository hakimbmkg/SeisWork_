...
closes the LOC-FLOW workflow (Zhang et al., 2022): events already in the
catalog are used as templates to scan continuous data and detect + locate
additional, usually smaller, events that the standard pick→associate→locate
chain misses. Detections can then be relocated relatively with GrowClust.

LOC-FLOW alur reproduced here (MatchLocate/run_matchlocate.sh):
...
The MatchLocate2 binary's internal SAC Butterworth bandpass (bpcc → xapiir,
order=4, passes=2) is reproduced in ObsPy when the SAC inputs are prepared
(detrend + taper + bandpass corners=4 zerophase=True). MatchLocate2 is then run
with -B-1/-1 (no internal filtering) — the same strategy verified bit-exact for
FDTCC, and which avoids vendoring the SAC signal-processing library that the
local MatchLocate2 source lacks (it links an xapiir guard stub instead).

To guarantee near-1.0 self-detection, templates are cut from the *same*
pre-filtered continuous SAC that is scanned (run_matchlocate.sh: "the ideal way
is to cut templates from filtered continuous data"), so template and trace share
filtering, sampling (0.01 s) and reference time exactly.
...
class MatchLocateDetection:
    ...
    def __init__(self, cfg: dict, base_dir: str):
        ...
        # SDS archive root: explicit data_source.sds_path → data.waveform_dir
        ...
        self.data_dir = os.path.join(self.out_dir, "Data")       # continuous SAC/day
        ...
        # bandpass: reuse the hypoDD crosscorr pre_filt band so templates and
        # continuous data are filtered identically to FDTCC.
        ...
        # MatchLocate2's Checkdata() requires identical sampling and begin time
        # across all continuous traces; the 7G archive mixes 100/200 Hz, so every
        # trace is interpolated onto one common day grid at target_sr.
    ...
    def _prep_continuous(self, ymd: str):
        ...
        every trace shares the same begin time (b=0) and length, and o=0 —
        ...
        for _, r in stadf.iterrows():
            ...
            # Unify sampling rate and begin time: pad generously, then resample
            # onto the common day grid (start = day0 exactly, npts identical).
            try:
    ...
    def _prep_template(self, ev: dict):
        """Cut the template window from the day's prepared continuous SAC and
        mark theoretical P/S arrivals (marktaup.py equivalent). Writes
        ...
        with open(inp_path, "w") as p:
            for f in sorted(os.listdir(ddir)):
                ...
                # first day-grid sample at/after tb, so b carries that sub-sample
                # offset and t1/t2 stay equal to the pure P/S travel times.
                s = tr.stats.sac
    ...
    def _select_final(self, ymd: str, det_files: list) -> list:
        """Merge per-template EventCase.out detections (SelectFinal.pl), dedup in
        time with the SelectFinal binary. Returns a list of detection dicts, each
        keeping the detecting template's name as ``ref`` so the GrowClust refine
            ...
            for f, template in det_files:
                ...
                for ln in rows[1:]:                       # skip header
                    ...
                    if int(float(ntr)) > mintrace:
                        ...
                        # before the template name. The template name carries a
                        # dot (YYYYMMDDhhmmss.ss); omitting ntrace would make %d
                        # swallow its integer part and %s grab only the fraction.
    ...
    def run(self, catalog_file: str):
        ...
        # 1) Prepare continuous data for every day we touch (template days ∪ scan).
        ...
        # 5) optional GrowClust relative relocation of the detections, following
        # LOC-FLOW's gen_input_matchlocate.pl (theoretical-phase hypoDD.pha).
    ...
    # ── template theoretical phases (from the template SAC t1/t2 headers) ──────
    def _template_phases(self, ref: str):
        """{station_code: (t1_P, t2_S)} read from Template/<ref>/NET.STA.CHA SAC
        headers. These are the theoretical travel times marked at template prep
        and reused as the detection's picks (gen_input_matchlocate.pl step 3)."""
        from obspy import read
    ...
    # ── GrowClust refinement (faithful LOC-FLOW gen_input_matchlocate.pl) ──────
    def _refine_growclust(self, dets: list):
        """Relocate the M&L detections relatively with GrowClust following
        LOC-FLOW's gen_input_matchlocate.pl: build a theoretical-phase hypoDD.pha
        (each detection's picks = its detecting template's P/S travel times),
