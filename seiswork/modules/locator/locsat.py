...
Converts catalog+picks to SCML with origins, runs screloc offline,
...
class LocSATLocator:
    ...
    def _find_seiscomp(self) -> str:
        ...
        if cfg_bin and os.path.exists(cfg_bin):
            return cfg_bin
        # User's SeisComP install: PATH first, then SEISCOMP_ROOT env, then the
        # standard ~/seiscomp and /opt/seiscomp locations. No personal paths.
    ...
    # ── Parse screloc SCML output (schema 0.14) into catalog DataFrame ───────
    ...
    def run(self, catalog_file: str):
        ...
        # Profile name: "iasp91" (matches ~/seiscomp/share/locsat/tables/).
        # Not "iaspei91" - that was the old incorrect name.
