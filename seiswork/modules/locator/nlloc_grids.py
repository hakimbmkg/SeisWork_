#!/usr/bin/env python3
"""
SeisWork — NonLinLoc travel-time grid generator (model GLOBAL IASP91)
Author : HakimBMKG

Builds NLLoc travel-time grids (TIME2D) for ANY station network at runtime,
using the 1-D IASP91 model + TRANS SIMPLE auto-centered on the active
network. Goal: portable NLLoc refinement for ANY network/region without
needing a pre-built, station-specific grid.
...
# Per-station TIME2D grid (distance-depth) — same dimensions as the bundled grid:
#   201 nodes × 5 km = 1000 km distance; 71 nodes × 5 km, origin -3 km = -3..352 km depth.
...
def auto_grid_dims(stations: list[dict]) -> tuple[int, int, float]:
    ...
    # Worst-case station-to-epicenter distance ~ network diameter (an event can occur
    # near any station within the network's footprint). Computed directly (rather
...
def station_fingerprint(stations: list[dict]) -> str:
    """Stable hash of the station set (code+lat+lon+elev) → grid cache key."""
    items = sorted(
...
def _gtsrce_block(stations: list[dict]) -> str:
    # GTSRCE <label> LATLON <lat> <lon> 0.0 <elev_km>. The label MUST equal the station
    # code used in the OBS file (NLLoc matches the grid prefix.PHASE.<label>.time).
    lines = []
    for s in stations:
...
def ensure_global_grids(stations: list[dict], cache_root: str,
    ...
    grid_ny/grid_nz/dxyz: None → auto-selected via `auto_grid_dims`
    (fine/small for regional networks, coarse/large for nationwide networks).
    center: TRANS SIMPLE origin (lat, lon); None → centroid of `stations`.
    When set (e.g. the midpoint of a user-drawn monitoring area) it is folded
    into the cache key so two grids over the same stations but different
    centers do not collide.
    Returns (grid_dir | None, prefix). grid_dir is ready to use as `grid_dir` +
    model_prefix=PREFIX in NLLocLocator.
    """
    fp = station_fingerprint(stations)
    if center is not None:
        # A different TRANS origin must yield a different cache dir — otherwise a
        # region-limited config would silently reuse a centroid-centered grid.
