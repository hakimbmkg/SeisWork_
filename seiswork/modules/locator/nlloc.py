...
Converts catalog → NLLoc PHASE format, runs NLLoc, parses .hyp output
...
class NLLocLocator:
    ...
    def __init__(self, cfg: dict, base_dir: str, stations: list[dict] | None = None):
        ...
        self.reg      = cfg["region"]
        # Optional station list ([{net,sta,lat,lon,...}]) — when given and no
        # travel-time grid exists yet at the configured grid_dir, run() builds
        # one automatically (auto-sized to the network's spread via
        # nlloc_grids.auto_grid_dims) instead of requiring it pre-built.
    ...
    # see GammaAssociator) — lets notebooks redirect output to a per-run working dir.
    @property
    ...
    def _find_exec(self, name: str) -> str:
        # PRIORITIZE the bundled binary in core/bin/ (populated by install.sh),
        # repo-relative → portable to other user installs. Only then fall back to PATH.
    ...
    def _grid_transform_origin(self, grid_dir: str, prefix: str):
        ...
        travel-time grids were generated (stored in their .hdr TRANSFORM
        line) — a mismatch silently shifts every located hypocenter by the
        lat/lon offset between the two origins. Returns None if no grid
        header is found (e.g. grids not built yet), so the caller can fall
        back to the region centre.
        """
    ...
    def _ensure_grid(self, grid_dir: str, prefix: str) -> tuple[str, str]:
        """When no travel-time grid exists yet at grid_dir/prefix and a station
        list was given, generate one — sized to the network's spread via
        nlloc_grids.auto_grid_dims (fine for a small regional network, coarser
        for a nationwide one) — instead of requiring it pre-built. Cached by
        station fingerprint, same as the realtime path (_ensure_global_grids
        in _realtime_pipeline.py). Returns (grid_dir, prefix), unchanged when
        a grid already exists or no stations were provided."""
    ...
    def _write_nlloc_ctrl(self, obs_file: str) -> str:
        ...
        # "Finished location"/"N events located" but writes ZERO output files
        # (no .hyp/.hdr/...), so _parse_hyp_files() always comes back empty.
        # Confirmed empirically with both MISFIT and OCT search methods.
        # `save_scatter` instead controls post-run cleanup of the bulky
        # per-event grid files (.buf/.scat/.angle) — see _cleanup_grid_files().
        save_flag = "SAVE"
        reg       = self.reg
        ...
        # as km offsets from the TRANS origin (1° lat ≈ 111 km;
        # 1° lon ≈ 111 km * cos(lat)) — correct even when the bbox centre and
        # the TRANS origin differ (origin is fixed by the travel-time grids).
        ...
        # Jailolo, ~100 km span → ~50 nodes/axis), but a NATIONWIDE region
        # (thousands of km) at a fixed 2 km would blow up to a search grid
        # with tens of millions of nodes — confirmed to make NLLoc's MISFIT
        # search take upwards of minutes/hang rather than run. Cap the
        # longer axis at MAX_NODES nodes and derive dx/dy/dz from that, same
        # spirit as nlloc_grids.auto_grid_dims's dxyz coarsening.
        MAX_NODES = 300
...
"""
    ...
    def _catalog_to_obs(self, catalog_file: str) -> str:
        ...
        # Per-event associated picks are written by the associator alongside
        # its catalog file (see GammaAssociator.run / RealAssociator.run), not
        # under base_dir/work/catalog — notebooks commonly redirect
        # catalog_dir to a per-run working directory.  `picks_associated.csv`
        # is the canonical name every associator writes (mirrors
        # `catalog_associated.csv`); `picks_gamma.csv` is kept as a fallback
        # for older run directories that predate that canonical name.
    ...
    def _cleanup_grid_files(self):
        ...
        SAVE is required for NLLoc to write .hyp at all (see _write_nlloc_ctrl),
        so this is how `save_scatter: false` keeps disk usage bounded without
        breaking the locator.
        """
    ...
    def _parse_hyp_files(self) -> pd.DataFrame:
        loc_dir = os.path.join(self.out_dir, "loc")
        # Besides one .hyp per event (loc.<date>.<time>.grid0.loc.hyp), NLLoc
        # also writes loc.sum.*.hyp (a running cumulative file containing
        # EVERY event so far) and last.hyp (a copy of the most recent event).
        # Globbing "*.hyp" picks up all three -> each event counted 2-3x.
        hyp_files = [
                    ...
                    for line in f:
                        ...
                        if line.startswith("GEOGRAPHIC"):
                            ...
                            try:
                                ...
                                lon = float(parts[11])
                                dep = float(parts[13])
                                # NLLoc prints hh/mm >= 24/60 for blunder solutions whose
                                # origin time crosses a day boundary without advancing the
                                # date — normalize via timedelta so the result is a valid
                                # datetime that pd.to_datetime() can parse.
    ...
    def run(self, catalog_file: str):
        ...
        # Preserve event_id from the INPUT catalog (e.g. GaMMA integer ids): the .hyp
        # id ("loc.YYYYMMDD...") breaks the link to picks_associated.csv
        # (event_index) required by HypoDD phase.dat & Result Viewer.
        # Match by nearest origin time (tolerance 60 s).
        try:
