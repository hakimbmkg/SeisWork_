...
class HypoDDRelocation:
    ...
    def _find_exec(self, name: str, cfg_name: str) -> str:
        # Prefer the bundled binary in core/bin/ (filled by install.sh), repo-relative.
    ...
    def _write_phase_dat(self, catalog_file: str, work_dir: str) -> str:
        cat    = pd.read_csv(catalog_file)
        # Look for picks next to the input catalog first (GUI: each job dir has
        # picks_associated.csv), then fall back to the old work/catalog.
        ...
        for picks_path in candidates:
            if os.path.exists(picks_path):
                ...
                break
        # GaMMA writes EVERY pick to picks_associated.csv, including the ~99%
        # not associated to an event (event_index = -1). Feeding all of them to
        # ph2dt builds a gigantic phase.dat and hangs. Keep only picks with
        # event_index >= 0 (e.g. 23k of 3.56M for the full-year run).
...
"""
    ...
    def _write_ph2dt_inp(self, phase_file: str, sta_file: str, work_dir: str) -> str:
        ...
        # User-edited override (previewed/edited in GUI before run), written verbatim.
    ...
    def _write_hypodd_inp(self, work_dir: str, use_cc: bool = False) -> str:
        ...
        getinp.f reads every non-comment line *positionally* (comments must
        start with '*' in column 1-2; blank lines are NOT skipped and still
        consume a slot). Required line sequence:
        ...
        A previous version wrote only 4 (mis-ordered) file names and an
        unrelated parameter layout; getinp.f kept reading positionally until
        it ran out of tokens mid-record -> "Fortran runtime error: End of
        file" at getinp.f:92.
        """
        inp_file = os.path.join(work_dir, "hypoDD.inp")
        # User-edited override (previewed/edited in GUI), written verbatim.
    ...
    def _compose_hypodd_inp(self, work_dir: str, use_cc: bool = False,
        ...
        """Build hypoDD.inp text. `relative=True` uses bare basenames (a
        portable preview, independent of the job dir); `relative=False` uses
        ...
        # run_hypodd_rekomendasi.ipynb): default 4 sets x 4 iterations, WRCT
        # tightens 8->3, WTCTS drops to 0.8 in the second half. CC fields -9 (unused).
        # Each set: (NITER, WTCCP, WTCCS, WRCC, WDCC, WTCTP, WTCTS, WRCT, WDCT, DAMP)
        #
        # NSET follows input `nset`: iteration blocks are generated NSET times,
        # WRCT interpolated linearly from `wrct_start` (loose) to `wrct_end`
        # (tight). An explicit `weighting_sets` list, if given, overrides this.
        ...
        if vel_model_key in _VELOCITY_MODELS:
            ...
            # Use the global-model vpvs as default; GUI override is still honored
            # (a user changing hd-vpvs away from the 1.730 default).
...
"""
    ...
    def generate_inp_preview(self) -> dict:
        ...
        so the result is portable (not tied to job dir). Only assembles text
        from current parameters for user review; does not run anything."""
    ...
    def _run_ph2dt(self, inp_file: str, work_dir: str):
        ...
        # absolute path (often 50-60+ chars here) silently truncates to 30
        # chars via getarg(), so ph2dt tries to open that truncated
    ...
    def _run_hypodd(self, inp_file: str, work_dir: str):
        ...
        # filename so the fixed-length Fortran argument buffer (fn_inp*80)
        # never risks truncating a long absolute path.
        with open(log, "w") as logf:
    ...
    def _write_fallback_relocated(self, catalog_file: str, reason: str,
        ...
        MINOBS) - e.g. short time windows with few co-recorded events - giving
        zero differential-time pairs and an empty hypoDD.reloc. Downstream
        stages still expect catalog_relocated.csv to exist, so fall back to
        the pre-relocation locations (tagged so the missing relocation is
        traceable) instead of crashing with a bare FileNotFoundError.
        """
    ...
    def run_catalog(self, catalog_file: str):
        ...
        if not df.empty:
            ...
            # hypodd_reloc.csv / catalog_relocated.csv there), then a
