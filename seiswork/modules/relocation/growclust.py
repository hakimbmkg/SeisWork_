...
dt.cc from FDTCC). Here we follow the same flow but reuse the catalog
...
Real waveform cross-correlation times, when available, can be dropped in as
dt.cc unchanged.
...
class GrowClustRelocation:
    ...
    def __init__(self, cfg: dict, base_dir: str):
        ...
        # phase.dat / ph2dt -> dt.ct) so both relative-location stages share
        # exactly the same inputs, matching the LOC-FLOW flow.
