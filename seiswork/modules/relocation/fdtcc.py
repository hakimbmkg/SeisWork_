...
(dt.cc) for hypoDD / GrowClust / tomoDD. This module replaces SeisWork's earlier
placeholder catalog-dt "dt.cc" with genuine waveform CC.

The FDTCC binary's internal SAC Butterworth bandpass (bpcc → xapiir, order=4,
passes=2) is reproduced here in ObsPy when the SAC inputs are prepared
(detrend + taper + bandpass corners=4 zerophase=True), after which FDTCC is run
with -B-1/-1 (no internal filtering). This was verified bit-exact against the
FDTCC Demo reference dt.cc, and avoids vendoring the SAC signal-processing
library (which the local FDTCC source lacks).

FDTCC inputs (LOC-FLOW alur):
...
class FDTCCDiffTimes:
    ...
    def __init__(self, cfg: dict, base_dir: str):
        ...
        # SDS archive root: explicit data_source.sds_path → waveform_dir
    ...
    def _write_station_dat(self, out_file: str, sta_bands: dict | None = None):
        ...
        code actually used for that station's SAC files in _prep_waveforms(),
        or FDTCC won't find the waveform. Falls back to the standard
    ...
    def _prep_waveforms(self, wav_dir: str, event_sel: str, comps=None, sta_bands=None):
        """For every day spanned by the events, fetch each station's traces from
        the SDS, apply the verified bandpass, and write SAC files whose reference
        time is the start of the day (origin O = 0 at day start, matching FDTCC's
        F=0 continuous convention).
        ...
        When both are None, the band is auto-detected per station —
        standard priority HH > BH > EH > SH — since a real network mixes
        sensor types.
        """
        import warnings
        from obspy import UTCDateTime
    ...
    def compute(self, work_dir: str) -> str:
        ...
        # Pre-filtered in ObsPy → run FDTCC with -B-1/-1 (no internal filter).
        cmd = [self.exec,
               "-F0", "-B-1/-1", "-C1/1/1",
               f"-W{self.win}",
