...
step8_service() {
    ...
    local sw_bin=""
    if ! $USE_VENV; then
        sw_bin="$("$CONDA" run -n "$ENV_NAME" which seiswork 2>/dev/null || true)"
        # Fall back through the real candidate paths and pick the first that EXISTS.
        # (The old code tested '[ -z "$sw_bin" ]' on the second fallback — dead code,
        # since the first fallback had already set a non-empty /opt/miniconda3 path even
        # when it doesn't exist, so miniforge installs got a bogus path → "not found".)
        if [ -z "$sw_bin" ] || [ ! -f "$sw_bin" ]; then
            for _cand in "/opt/miniconda3/envs/$ENV_NAME/bin/seiswork" \
...
main() {
    ...
    if $USE_VENV; then
        ...
        echo "  Run SeisWork (manual):"
        echo "    conda activate $ENV_NAME && seiswork gui"
    fi
    echo
