...
Usage:
    ...
    python agent.py --port 7001     # custom port
...
# ── Minimal dependencies (stdlib + Flask) ──────────────────────────────────
try:
...
_seiswork_token = None     # SeisWork token
...
def _seiscomp_env() -> dict:
    """Env for SeisComP subprocesses, without this agent's own Python stack.
    The agent's conda env often ships numpy 2, but SeisComP's compiled Python
    modules need numpy 1 and crash with '_ARRAY_API not found' (seiscomp
    update-config exits rc=1, though it works fine from a login shell). Put
    system dirs first so python3 resolves to SeisComP's own interpreter, and
    drop interpreter-hijacking env vars."""
    env = dict(os.environ)
    conda_markers = ("conda", "virtualenv", "/venv", "/.venv")
    parts = [p for p in env.get("PATH", "").split(os.pathsep)
             if p and not any(m in p.lower() for m in conda_markers)]
    for sysdir in ("/usr/local/bin", "/usr/bin", "/bin"):
        if sysdir not in parts:
            parts.append(sysdir)
...
def _slinktool_streams(host: str, port: int, timeout: float = 15.0):
    """Query streams via the `slinktool -Q` binary (ships with SeisComP).
    More robust than hand-parsing raw SeedLink INFO packets (MiniSEED framing
    corrupts a naive offset-8 parse into 'not well-formed' XML). Returns a
    stream list, or None if slinktool fails, so the caller falls back to a
    raw-socket parse.
...
def _query_seedlink_streams(host: str, port: int, timeout: float = 8.0) -> list[dict]:
    ...
    # Prefer slinktool; the raw INFO parse below mis-handles MiniSEED framing on
    # some SeisComP seedlink builds ('not well-formed' XML at ~col 433).
    ...
    try:
        ...
        # INFO STREAMS: response is a SeedLink info packet (512-byte records ending with END)
        ...
        # Extract XML from raw bytes: 8-byte SEED header + 512-byte record, skip non-XML bytes
        xml_parts = []
        i = 0
        while i < len(raw):
...
def _json_error(e):
    """Every consumer here is a machine (SeisWork GUI) parsing JSON. Flask's
    default HTML 500 page turned bugs into unreadable garbage in the Sync log.
    Return the real exception as JSON instead, and keep the traceback in the
    agent log."""
    import traceback
...
def sync_seiscomp():
    """
    Full workflow: query seedlink -> generate inventory + bindings -> push to
    SeisComP -> restart.
    ...
    # 1. Discover streams from seedlink (optional; continue with inventory on failure)
    streams = []
    try:
    ...
    # Detect format: FDSN StationXML vs SeisComP XML
    if inv_xml:
    ...
    # Source: discovered streams if available, else parsed inventory XML (FDSN or SC3)
    # SEED codes only (letters/digits, <=8 chars): filename is station_{NET}_{STA}.
    # Bug seen once: a SeisComP publicID like "Station/20260512100931..." leaked in
    # as a code, turning the filename into a nested path and crashing write_text().
    import re as _re_codes
    _valid_code = _re_codes.compile(r"^[A-Za-z0-9]{1,8}$").match

    def _add_net_sta(net_code: str, sta_code: str):
        if _valid_code(net_code or "") and _valid_code(sta_code or ""):
            net_sta_seen.add((net_code, sta_code))

    net_sta_seen = set()
    if streams:
        for s in streams:
            _add_net_sta(s["net"], s["sta"])
    elif inv_xml:
        try:
            ...
            for _net_el in _root_xml.iter(f"{{{_fdsn_ns}}}Network"):
                ...
                for _sta_el in _net_el.iter(f"{{{_fdsn_ns}}}Station"):
                    _add_net_sta(net_code or "IA", _sta_el.get("code", ""))
            if not net_sta_seen:
                # SeisComP XML fallback: sc3ml nests <network code=..><station code=..>
                # (lowercase tags, any namespace). Old parse derived network from the
                # station's publicID (an opaque id, not a code), producing the
                # garbage filenames above; use the code attributes instead.
                for _net_el in _root_xml.iter():
                    if not (_net_el.tag.endswith("}network") or _net_el.tag == "network"):
                        continue
                    net_code = _net_el.get("code", "")
                    for _sta_el in _net_el.iter():
                        if _sta_el.tag.endswith("}station") or _sta_el.tag == "station":
                            _add_net_sta(net_code or "IA", _sta_el.get("code", ""))
        except Exception:
            pass

    # Channel per station, priority: (1) Z channel confirmed live from discovery,
    # (2) default_channel from meta config, (3) fallback "BHZ". Hardcoding "BHZ" is
    # wrong for stations with only short-period sensors (SHZ). Band priority matches
    # seiswork.utils.channels.BAND_PRIORITY (HH>BH>EH>SH); duplicated here since this
    # file must stay stdlib-only for standalone install on the SeisComP host.
    live_channel = {}
    if streams:
    ...
    for net_code, sta_code in net_sta_seen:
        ...
        try:
            ...
            key_errors.append(f"{net_code}.{sta_code}: {e}")
    steps.append({"step": "bindings", "ok": len(written_keys) > 0,
                   "count": len(written_keys),
                   **({"errors": key_errors[:5]} if key_errors else {})})

    # 4. Update SeisComP config (scales with station count; 1000+ bindings can
    # exceed 30s)
    rc, out, err = _run_seiscomp("update-config", timeout=180)
    steps.append({"step": "update_config", "ok": rc == 0, "rc": rc,
                   "stderr": (err or "")[:300] if rc != 0 else ""})
    ...
    # Minimal success: bindings written (local setup done).
    # inventory_sync & update_config need scmaster running; optional.
...
def push_config():
    ...
    data = request.get_json(force=True) or {}
    # e.g.: {"scautopick.staSta": 0.5, "scautopick.ltaLen": 10.0}
...
# A SeisWork on another host can push its own located events (origins,
# arrivals, picks) into this SeisComP's messaging via scdispatch, landing in
# scevent under SeisWork's own agencyID. Remote counterpart of app.py's local
# api_inject_seiscomp: the SC binary runs here, so seiswork itself never
# needs SeisComP installed, only reachability to this port + the token.
...
# ── Bridge Thread: Subscribe SeisComP message bus, POST picks to SeisWork ──
def _bridge_loop():
    ...
    # Try importing seiscomp.client (only in the SeisComP env; can also fail with
    # AttributeError/SystemError from a NumPy ABI mismatch between the system
    # python that compiled _core and this agent's conda env).
    try:
...
def _bridge_db_poll(req_lib):
    ...
    # _last_modified uses MySQL's current_timestamp(), the server's local time
    ...
    while _bridge_running and _seiswork_url:
        try:
            ...
            with conn.cursor() as cur:
                # SeisComP schema: columns prefixed waveformID_*; insert timestamp
                # is _last_modified (Pick table has no "created" column).
                cur.execute("""
