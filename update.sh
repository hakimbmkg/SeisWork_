...
restart_server(){
    ...
    if systemctl --user list-unit-files 2>/dev/null | grep -q '^seiswork\.service'; then
        systemctl --user restart seiswork 2>/dev/null
        # `systemctl restart` on a Type=simple unit returns as soon as the new
        # process is forked -- it does NOT wait to see whether that process
        # then immediately exits (e.g. status=127 from a broken ExecStart), so
        # its own exit code is not a reliable health signal on its own. Give
        # it a moment, then check the actual unit state.
        sleep 3
        if [ "$(systemctl --user is-active seiswork 2>/dev/null)" = "active" ]; then
            ok "service 'seiswork' restarted"
            return
        fi
        # Not active — check for a conda/PATH crash-loop (status=127)
        warn "Service did not come up active — checking for a crash-loop..."
        if journalctl --user -u seiswork -n 15 --no-pager 2>/dev/null \
            ...
            warn "Detected a 'conda not found' crash-loop — invoking web_service.sh ..."
            local FIX="$DIR/tools/web_service.sh"
            if [ -f "$FIX" ]; then
                bash "$FIX" && return
            else
                warn "web_service.sh not found at $FIX"
                warn "For another VM: bash tools/web_service.sh --print  (copy-paste onto the VM)"
            fi
        else
...
SeisWork updater — current version: $(ver_get)
  ...
  bash tools/web_service.sh          Run on a machine whose service is crash-looping
  bash tools/web_service.sh --print  Print a ready-to-paste block for another VM (no SSH)
EOF
}
...
# Documentation — auto-regenerate whenever update.sh actually applies changes
# (best-effort; mirrors install.sh's generate_docs but uses sw_python() since
# update.sh doesn't share install.sh's ENV_NAME/VENV_DIR/USE_VENV variables).
generate_docs(){
    local gen="$DIR/docs/generate_full_doc.py"
    [ -f "$gen" ] || return 0
    local PY; PY="$(sw_python)"
    [ -n "$PY" ] || return 0
    "$PY" -m pip install markdown weasyprint -q &>/dev/null
    "$PY" "$gen" \
        && ok "Documentation regenerated in docs/" \
...
if ! git pull --ff-only origin "$BR"; then
    # A public-repo re-release publishes a fresh orphan history (no shared
    # ancestor with what's checked out here) — that's not a real conflict,
    # ff-only just can't ever work for it. Detect that case (no merge-base)
    # and reset onto origin instead of failing; a genuine conflict (shared
    # history, real divergence) still stops with an error for manual review.
    MERGE_BASE="$(git merge-base HEAD "origin/${BR}" 2>/dev/null || true)"
    if [ -z "$MERGE_BASE" ]; then
        warn "origin/${BR} has an unrelated history (repo was re-published) — resetting local branch to match it."
        if ! git reset --hard "origin/${BR}"; then
            err "git reset --hard origin/${BR} failed. Resolve manually then retry."
            [ "$STASHED" = 1 ] && git stash pop >/dev/null 2>&1 || true
            exit 1
        fi
    else
        err "git pull failed (likely a real conflict). Resolve manually then retry."
        [ "$STASHED" = 1 ] && git stash pop >/dev/null 2>&1 || true
        exit 1
    fi
fi
